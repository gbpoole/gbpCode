...
void compute_MCMC(MCMC_info *MCMC) {
    ...
    if(SID.I_am_Master) {
        ...
        // ... else if run.dat does not exist, create it
            ...
            if((fp_run = fopen(filename_run, "wb")) == NULL)
                SID_exit_error("Could not open file for writing {%s}.", SID_ERROR_IO_OPEN, filename_run);
    ...
    if(!flag_restart) {
        ...
        if(my_chain == SID.My_rank) {
            ...
            for(i_P = 0; i_P < n_P; i_P++)
                SID_log(format_string,
                        SID_LOG_ALLRANKS | SID_LOG_COMMENT,
                        MCMC->P_names[i_P],
                        P_last[i_P],
                        MCMC->P_limit_min[i_P],
                        MCMC->P_limit_max[i_P]);
        ...
        if((fp_chain = fopen(filename_chain, "wb")) == NULL)
            SID_exit_error("Could not open file for writing {%s}.", SID_ERROR_IO_OPEN, filename_chain);
        if((fp_stats = fopen(filename_stats, "wb")) == NULL)
            SID_exit_error("Could not open file for writing {%s}.", SID_ERROR_IO_OPEN, filename_stats);
        if((fp_chain_config = fopen(filename_chain_config, "wb")) == NULL)
            SID_exit_error("Could not open file for writing {%s}.", SID_ERROR_IO_OPEN, filename_chain_config);
    ...
    for(; i_phase < 2; i_phase++) {
        ...
        while(flag_continue) {
            ...
            if(my_chain == SID.My_rank) {
                ...
                compute_MCMC_chain_stats(P_prop_stats,
                                         n_P,
                                         n_avg,
                                         P_min,
                                         P_avg,
                ...
                compute_MCMC_chain_stats(P_chain_stats,
                                         n_P,
                                         n_avg,
                                         P_min,
                                         P_avg,
        ...
        // Report success rate
        SID_log("Proposal success: %5.3f%% (%lld of %lld)",
                SID_LOG_COMMENT,
                1e2 * ((float)(MCMC->n_success) / (float)(MCMC->n_propositions)),
                MCMC->n_success,
                MCMC->n_propositions);
