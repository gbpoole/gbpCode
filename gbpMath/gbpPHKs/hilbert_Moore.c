...
#define setBytes(dst, pos, nBytes, val) memset(&dst[pos + 1], val, nBytes - pos - 1)
#else
#define whichByte(nBytes, y) (y / 8)
#define setBytes(dst, pos, nBytes, val) memset(&dst[0], val, pos)
#endif
...
#include <string.h>
static void propogateIntBits(unsigned d, unsigned nBytes, char *c, unsigned y, int fold) {
...
static bitmask_t getIEEESignBits(unsigned nDims, double const *c) {
    unsigned       d;
    ieee754_double x;
    bitmask_t      bits = 0;
    for(d = 0; d < nDims; ++d) {
...
static bitmask_t getIEEEBits(unsigned    nDims,
...
static void propogateIEEEBits(unsigned d, unsigned nBytes, char *cP, unsigned y, int fold) {
...
static unsigned getIEEEexptMax(unsigned nDims, double const *c) {
    unsigned max = 0;
    unsigned d;
    for(d = 0; d < nDims; ++d) {
        ieee754_double x;
...
static void getIEEEinitValues(double const *c1, unsigned y, unsigned nDims, unsigned *rotation, bitmask_t *bits, bitmask_t *index) {
...
static int hilbert_cmp_work(unsigned    nDims,
                            unsigned    nBytes,
                            unsigned    nBits,
                            unsigned    max,
                            unsigned    y,
...
static unsigned hilbert_box_vtx_work(unsigned  nDims,
                                     unsigned  nBytes,
                                     unsigned  nBits,
                                     int       findMin,
                                     unsigned  max,
 ...
 *      nBits <= (sizeof bitmask_t) * (bits_per_byte)
 */
unsigned hilbert_box_pt_work(unsigned  nDims,
                             unsigned  nBytes,
                             unsigned  nBits,
                             int       findMin,
                             unsigned  max,
...
unsigned hilbert_box_pt(unsigned nDims, unsigned nBytes, unsigned nBits, int findMin, void *c1, void *c2) {
    bitmask_t const one  = 1;
    bitmask_t       bits = one << (nDims - 1);
...
unsigned hilbert_ieee_box_pt(unsigned nDims, int findMin, double *c1, double *c2) {
    ...
    if(c1Signs != c2Signs) {
        ...
        index    = 1;
        hilbert_box_pt_work(
            nDims, 8, 64, findMin, IEEErepBits - 1, IEEErepBits, (char *)c1, (char *)c2, rotation, bits, index, getIEEEBits, propogateIEEEBits);
    }
 ...
 *      nBits <= (sizeof bitmask_t) * (bits_per_byte)
 */
int hilbert_nextinbox(unsigned nDims, unsigned nBytes, unsigned nBits, int findPrev, void *c1V, void *c2V, void const *ptV) {
    bitmask_t const one      = 1;
    unsigned        y        = nBits;
    bitmask_t const ndOnes   = ones(bitmask_t, nDims);
    bitmask_t const nd1Ones  = ndOnes >> 1;
