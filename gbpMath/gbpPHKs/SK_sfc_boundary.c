 ...
 */
inline static uint32_t local_localize_key(sfc_key_t key, uint32_t ncpu, sfc_key_t *fstkey, sfc_key_t *lstkey);
...
extern sfc_boundary_t
    sfc_boundary_new(uint32_t num, sfc_boundary_type_t btype, sfc_key_t minkey, sfc_key_t maxkey, uint32_t bits, sfc_curve_t ctype) {
    ...
    if(dummy == NULL) {
        SID_trap_error("boundary array", ERROR_LOGIC);
        /*
...
extern sfc_boundary_2_t sfc_boundary_2_get(uint32_t rank, uint32_t ncpu, sfc_key_t *fstkey, sfc_key_t *lstkey, uint32_t bits, sfc_curve_t ctype) {
    ...
    if(dummy == NULL) {
        SID_trap_error("boundary structure", ERROR_LOGIC);
        /*
...
extern sfc_boundary_t sfc_boundary_get(sfc_boundary_type_t btype, sfc_key_t minkey, sfc_key_t maxkey, uint32_t bits, sfc_curve_t ctype) {
    ...
    if((btype != SFC_BOUNDARY_TYPE_INNER) && (btype != SFC_BOUNDARY_TYPE_OUTER)) {
        SID_trap_error("unsupported boundary type", ERROR_LOGIC);
        /*
...
return NULL;
    ...
    if(dummy->bound == NULL) {
        free(dummy);
        SID_trap_error("boundary key array", ERROR_LOGIC);
        /*
...
extern sfc_boundary_t sfc_boundary_addKey(sfc_boundary_t bound, sfc_key_t key) {
    ...
    if(sfc_boundary_findKeyPos(bound, key, &pos))
        return bound;
    ...
    if(bound->num >= bound->len) {
        ...
        */
        ...
        if(tmp == NULL) {
            SID_trap_error("resized boundary array", ERROR_LOGIC);
            /*
    ...
    */
    ...
    if(bound->num > pos) {
        memmove(bound->bound + pos + UINT64_C(1), bound->bound + pos, (bound->num - pos) * sizeof(sfc_key_t));
    }
    bound->bound[pos] = key;
    bound->num++;
...
io_logging_msg(log, INT32_C(15),
        ...
        if(bound->bound[pos] < key) {
            if((pos == bound->num - UINT64_C(1)) || (bound->bound[pos + UINT64_C(1)] > key)) {
...
*/
            ...
            if((pos == UINT64_C(0)) || (bound->bound[pos - UINT64_C(1)] < key)) {
