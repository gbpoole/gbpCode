...
int swap_endian_snapshot(const char *filename_in_root,
                         const char *filename_out_root,
                         int         region_number,
                         int         snap_number,
                         int         mode,
                         int *       IDs_byte_size_return) {
    if(region_number < 0)
    ...
    if(region_number < 0) {
        ...
        sprintf(filename_in,
                "%s/snapshots/snapshot_region%03d_%03d/snapshot_region%03d_%03d.%d",
                filename_in_root,
                region_number,
                snap_number,
                region_number,
    ...
    int                i_file_in;
    int                n_files;
    SID_fread_verify(&block_size_in, sizeof(int), 1, fp_in);
    SID_fread_verify(&header, sizeof(gadget_header_info), 1, fp_in);
    SID_fread_verify(&block_size_out, sizeof(int), 1, fp_in);
    ...
    for(int i_file = 0; i_file < n_files && flag_ID_size_not_set; i_file++) {
        ...
        if(n_particles_file > 0) {
            ...
            if(n_particles_file == (block_size_in / sizeof(int)))
                ...
                SID_exit_error("Can not determine particle ID size from IDs block size (%d).", SID_ERROR_LOGIC,
                               block_size_in);
