    ...
    if(halo != NULL) {
        ...
        if(halo->parent_top == NULL) {
            if(trees->group_match_scores != NULL)
                ...
                SID_exit_error("Group match scores are not defined.  They probably have not been read.",
                               SID_ERROR_LOGIC);
        } else {
            if(trees->subgroup_match_scores != NULL)
                match_score = trees->subgroup_match_scores[halo->snap_tree][halo->neighbour_index];
            else
                SID_exit_error("Subgroup match scores are not defined.  They probably have not been read.",
                               SID_ERROR_LOGIC);
        }
