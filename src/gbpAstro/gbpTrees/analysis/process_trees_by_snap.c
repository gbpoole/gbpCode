...
void process_trees_by_snap(tree_info *trees,
    ...
    if(i_snap_hi < i_snap_lo || i_snap_hi > trees->n_snaps || i_snap_lo < 0)
        SID_exit_error("Invalid snapshot range (%d to %d; %d in trees) passed to process_trees_by_snap().",
                       SID_ERROR_LOGIC, i_snap_lo, i_snap_hi, trees->n_snaps);
    ...
    for(int i_type = 0; i_type < 2; i_type++) {
        ...
        if(flag_do) {
            ...
            int flag_init_init_snap = GBP_TRUE;
            int flag_init_analyze   = GBP_TRUE;
            int flag_init_select    = GBP_TRUE;
            int flag_init_fin_snap  = GBP_TRUE;
