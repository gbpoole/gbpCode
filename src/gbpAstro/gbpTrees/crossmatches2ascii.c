...
int main(int argc, char *argv[]) {
    ...
    for(int i_read = 0; i_read < 2; i_read++) {
        ...
        SID_log("n_groups_i=%d", SID_LOG_COMMENT, n_groups_1);
        SID_fread(&n_groups_2, sizeof(int), 1, &fp_in);
        SID_log("n_groups_j=%d", SID_LOG_COMMENT, n_groups_2);
        if(i_read == 0) {
            n_groups_i = n_groups_1;
            n_groups_j = n_groups_2;
        } else if(n_groups_i != n_groups_2 || n_groups_j != n_groups_1)
            SID_exit_error("Input match file halo counts are not symetric (ie %d!=%d or %d!=%d).", SID_ERROR_LOGIC,
                           n_groups_i, n_groups_1, n_groups_j, n_groups_2);
