...
void compute_trees_horizontal(char *       filename_halo_root_in,
    ...
    SID_log("Constructing horizontal merger trees for snapshots #%d->#%d (step=%d)...",
    ...
    double *a_list = NULL;
    int     n_a_list_in;
    write_a_list(filename_snap_list_in, filename_output_dir, i_read_start, i_read_stop, i_read_step);
    read_a_list(filename_output_dir, &a_list, &n_a_list_in);
    ...
    for(i_read = i_read_stop - i_read_step, i_file = i_file_start - 1, j_file = 1, i_write = i_file_start, j_write = i_read_stop, l_write = 0;
        i_read >= i_read_start;
        ...
        for(k_match = 0; k_match < n_k_match; k_match++) {
            ...
            finalize_trees_horizontal(n_halos_1_matches, n_halos_i, halos, halos_i, i_file, n_search, n_wrap, &max_id, &max_tree_id);
