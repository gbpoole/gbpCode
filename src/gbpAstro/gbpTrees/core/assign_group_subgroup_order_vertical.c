    ...
    if(n_neighbours > 0) {
        ...
        while(current != NULL) {
            if(i_halo >= n_neighbours)
                SID_exit_error(
                        "There's a problem with the number of neighbours in assign_group_subgroup_order (%d>=%d)!",
                        SID_ERROR_LOGIC, i_halo, n_neighbours);
        ...
        if(i_halo != n_neighbours)
            SID_exit_error("There's a problem with the number of neighbours in assign_group_subgroup_order (%d!=%d)!",
                           SID_ERROR_LOGIC, i_halo, n_neighbours);
                ...
                if(n_in_group > 1) {
                    ...
                    for(j_halo = i_halo, k_halo = 0; k_halo < n_in_group; j_halo++, k_halo++) {
                        if(neighbours[group_ids_index[j_halo]] != new_last)
                            neighbours[group_ids_index[j_halo]]->group_halo_next =
                                neighbours[group_ids_index[i_halo + halo_score_index[halo_score_rank[k_halo] - 1]]];
                        else
