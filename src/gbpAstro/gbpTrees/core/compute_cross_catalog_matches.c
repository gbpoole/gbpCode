...
int compute_cross_catalog_matches(char *filename_root_in_1,
    ...
    if(!strcmp(filename_root_in_1, filename_root_in_2))
        ...
        SID_log("Processing matches between catalog;snap's {%s;%03d} and {%s;%03d}...",
                SID_LOG_OPEN | SID_LOG_TIMER,
                filename_root_in_1,
                i_read_1,
                filename_root_in_2,
    ...
    read_groups(filename_root_in_2,
                i_read_2,
                READ_GROUPS_ALL | READ_GROUPS_NOPROPERTIES | READ_GROUPS_PEANOHILBERT,
                &plist2,
                filename_cat2,
                PHK_min_local,
                PHK_max_local);
    ...
    if(n_bits_PHK_1 != n_bits_PHK_2)
        SID_exit_error("The PHK bit-sizeo of the two catalog's precomputed PHKs don't match (ie %d!=%d).",
                       SID_ERROR_LOGIC, n_bits_PHK_1, n_bits_PHK_2);
