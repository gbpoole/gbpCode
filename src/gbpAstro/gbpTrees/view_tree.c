...
int main(int argc, char *argv[]) {
    ...
    SID_fread_verify(&n_trees, sizeof(int), 1, fp);
    SID_fread_verify(&n_halos_total, sizeof(int), 1, fp);
    SID_log("(%d trees and %d halos)...", SID_LOG_CONTINUE, n_trees, n_halos_total);
    n_halos = (int *)SID_malloc(sizeof(int) * n_trees);
    SID_fread_verify(n_halos, sizeof(int), n_trees, fp);
    for(i_tree = 0; i_tree < select_tree; i_tree++) {
        for(i_halo = 0; i_halo < n_halos[i_tree]; i_halo++) {
            SID_fread_verify(&halo, sizeof(halo_properties_SAGE_info), 1, fp);
            max_snap = GBP_MAX(max_snap, halo.snap_num);
        }
    }
    ...
    SID_fread_verify(halos, sizeof(halo_properties_SAGE_info), n_halos[i_tree], fp);
    ...
    for(i_halo = 0; i_halo < n_halos[i_tree]; i_halo++) {
        ...
        max_snap             = GBP_MAX(max_snap, halos[i_halo].snap_num);
    }
            ...
            */
            /*
                       printf("%05d/%05d/%05d",
                              ...
                              halos[k_halo].n_particles);
            */
            printf("%05d/%05d/%05d/%05d", k_halo, halos[k_halo].descendant, halos[k_halo].group_halo_first, halos[k_halo].group_halo_next);
