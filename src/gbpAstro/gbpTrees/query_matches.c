...
int main(int argc, char *argv[]) {
    ...
    SID_log("snap start  =%d", SID_LOG_COMMENT, i_read_start);
    SID_fread(&i_read_stop, sizeof(int), 1, &fp_in);
    SID_log("snap stop   =%d", SID_LOG_COMMENT, i_read_stop);
    SID_fread(&n_search, sizeof(int), 1, &fp_in);
    SID_log("search range=%d", SID_LOG_COMMENT, n_search);
    SID_fread(&n_files, sizeof(int), 1, &fp_in);
    SID_log("# of files  =%d", SID_LOG_COMMENT, n_files);
    ...
    for(j_read = GBP_MIN(i_read_stop, i_read + n_search); j_read >= GBP_MAX(0, i_read - n_search); j_read--) {
        if(i_read != j_read) {
            ...
            if(check_validity_of_match(n_particles_i[i_halo], match_count[i_halo], match_score[i_halo], F_MATCH_MOMENT_DIFF_MIN_DEFAULT))
            ...
            if(match >= 0)
                fprintf(fp_out,
                        "%3d %7d %6d %3d %7d %6d %10.3le %7d %10.3le %10.3e %s %s\n",
                        i_read,
                        i_halo,
