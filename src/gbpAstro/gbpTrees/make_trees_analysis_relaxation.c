    ...
    for(int i_M = 0; i_M < n_M; i_M++) {
        ...
        if(params->flag_calc_Vir)
            for(int i_z = 0; i_z < n_z; i_z++)
                fprintf(fp_out,
                        "%3d %6.2lf %6d %6d %6d %6d %6d %6d %6d %6d\n",
                        trees->snap_list[i_z],
                        trees->z_list[i_z],
                        params->n_halos_z[i_M][i_z],
                        params->n_halos_z_recovered_form[i_M][i_z],
...
int main(int argc, char *argv[]) {
    ...
    // Perform analysis
    tree_info *trees;
    read_trees(filename_SSimPL_root, filename_halo_version_root, filename_trees_name, TREE_MODE_DEFAULT, &trees);
    ...
    SID_log("Trends with tau will be compiled between (z,snap)=(%lf,%d) and (z,snap)=(%lf,%d).",
            SID_LOG_COMMENT,
            trees->z_list[params.snap_hi_tau_trends],
            params.snap_hi_tau_trends,
            trees->z_list[params.snap_lo_tau_trends],
            params.snap_lo_tau_trends);

    // ** PERFORM the calculation here **
    process_trees_by_snap(trees,
                          &params,
                          PROCESS_TREES_BOTH,
                          0,
                          trees->n_snaps,
