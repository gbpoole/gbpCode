    ...
    for(int i_type = 0; i_type < N_GADGET_TYPE; i_type++) {
        if(ADaPS_exist(plist->data, "n_all_%s", plist->species[i_type])) {
            ...
            for(int i_rank = 0; i_rank < SID.n_proc; i_rank++) {
                ...
                SID_Bcast(&n_of_type, 1, SID_SIZE_T, i_rank, SID_COMM_WORLD);
                ...
                if(SID.My_rank == i_rank || SID.I_am_Master) {
                    ...
                    for(size_t j_particle = 0; j_particle < n_of_type;) {
                        ...
                        if(SID.I_am_Master) {
                            ...
                            for(size_t k_particle = 0; k_particle < n_buffer; k_particle += n_write_file,
                                ...
                                if(n_variables > 1) {
                                    ...
                                    for(size_t k_buffer = 0; k_buffer < n_buffer; k_buffer++) {
                                        for(size_t i_variable = 0; i_variable < n_variables;
                                            i_buffer += write_size[i_variable], j_buffer[i_variable] += write_size[i_variable], i_variable++)
                ...
                SID_Bcast(&n_of_type_processed, 1, SID_SIZE_T, i_rank, SID_COMM_WORLD);
                SID_Bcast(&n_of_type_file_left, 1, SID_SIZE_T, i_rank, SID_COMM_WORLD);
            } // i_rank
            if(fp_out != NULL) {
                fwrite(&block_size, sizeof(int), 1, fp_out);
                fclose(fp_out);
...
void write_gadget_binary_new(plist_info *plist, char *filename_out_root, int n_files, int mode) {
    ...
    convert_mode[1] = 0;
    convert_mode[2] = 0;
    write_gadget_binary_block(plist, n_files, filename_out_root, n_variables, storage_size, variable, convert_mode, mode);
    ...
    /*
      ...
      for(int i_type=0;i_type<N_GADGET_TYPE;i_type++){
         for(int i_rank=0;i_rank<SID.n_proc;i_rank++){
            ...
            SID_Bcast(&last_ID,1,SID_SIZE_T,i_rank,SID_COMM_WORLD);
         }
      }
      SID_log("Done.",SID_LOG_CLOSE);
    */
