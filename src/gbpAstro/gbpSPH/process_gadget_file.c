...
void process_gadget_file(const char *status_message,
    ...
    for(int j_file = 0; i_file < fp_gadget.header.n_files; i_file += i_file_skip, j_file++) {
        ...
        record_length = 3 * n_particles_file * sizeof(GBPREAL);
        SID_fread_verify(&record_length_open, 4, 1, fp_ids);
        fseeko(fp_ids, (off_t)(record_length), SEEK_CUR);
        SID_fread_verify(&record_length_close, 4, 1, fp_ids);
        // if(record_length_open!=record_length_close)
        ...
        for(i_type = 0, i_particle = 0; i_type < N_GADGET_TYPE; i_type++) {
            for(size_t j_particle = 0; j_particle < header.n_file[i_type]; i_particle += n_buffer, j_particle += n_buffer) {
                ...
                if(flag_I_am_reader) {
                    SID_fread_verify(pos_buffer, sizeof(GBPREAL), 3 * n_buffer, fp_pos);
                    SID_fread_verify(vel_buffer, sizeof(GBPREAL), 3 * n_buffer, fp_vel);
                    SID_fread_verify(ids_buffer, ID_byte_size, n_buffer, fp_ids);
                }
                if(flag_all_to_all) {
                    SID_Bcast(pos_buffer, 3 * n_buffer, SID_REAL, all_to_all_reader_rank, SID_COMM_WORLD);
                    SID_Bcast(vel_buffer, 3 * n_buffer, SID_REAL, all_to_all_reader_rank, SID_COMM_WORLD);
                    SID_Bcast(ids_buffer, ID_byte_size * n_buffer, SID_CHAR, all_to_all_reader_rank, SID_COMM_WORLD);
                }
                ...
                SID_Allreduce(&flag_active, &n_ranks_active, 1, SID_INT, SID_SUM, SID_COMM_WORLD);
                SID_Allreduce(&n_particles_processed_local, &n_particles_processed, 1, SID_SIZE_T, SID_SUM, SID_COMM_WORLD);
    ...
    SID_Allreduce(n_particles_type_local, n_particles_type, N_GADGET_TYPE, SID_SIZE_T, SID_SUM, SID_COMM_WORLD);
