    ...
    if(image != NULL) {
        ...
        for(int i_w = 0; i_w < image->width; i_w++) {
            for(int i_h = 0; i_h < image->height; i_h++) {
                ...
                if(image_i > image_max)
                    image_max = image_i;
                if(image_i < image_min && image_i != LOG_ZERO)
                    image_min = image_i;
            }
        }
        ...
        // Write raw image
        if(SID_CHECK_BITFIELD_SWITCH(mode, WRITE_IMAGE_RAW)) {
        ...
        // Write png
        if(SID_CHECK_BITFIELD_SWITCH(mode, WRITE_IMAGE_PNG)) {
