...
void read_image(image_info *image, const char *filename_dir, const char *filename_root) {
    ...
    SID_fread_verify(&(image->width), sizeof(int), 1, fp);
    SID_fread_verify(&(image->height), sizeof(int), 1, fp);
    image->n_pixels = image->width * image->height;
    SID_fread_verify(image->values, sizeof(double), image->n_pixels, fp);
    fclose(fp);
