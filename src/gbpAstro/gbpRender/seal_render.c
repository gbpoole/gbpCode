...
void seal_render(render_info *render) {
    if(!render->sealed) {
        ...
        if(strcmp(render->snap_a_list_filename, "none")) {
            ...
            SID_Bcast(&(render->n_snap_a_list), 1, SID_INT, SID_MASTER_RANK, SID_COMM_WORLD);
            ...
            SID_Bcast(render->snap_a_list, render->n_snap_a_list, SID_DOUBLE, SID_MASTER_RANK, SID_COMM_WORLD);
        }
