...
void merge_sort(void *data_in, size_t n_data, size_t **index, SID_Datatype data_type, int flag_compute_index, int flag_in_place) {
    size_t  i, j;
    void *  data_sort;
    void *  scratch_d;
    size_t *scratch_i;
    ...
    if(flag_compute_index == SORT_COMPUTE_INDEX || flag_compute_index == SORT_COMPUTE_RANK) {
        ...
        fprintf(stderr,
                "ERROR (merge_sort): flag_compute_index {%d} must be SORT_COMPUTE_INDEX||SORT_COMPUTE_RANK||SORT_INPLACE_ONLY\n",
                flag_compute_index);
    }
    ...
    // Perform the sort
    if(n_data > 0)
        merge_helper(data_sort, idx_sort, 0, n_data, scratch_d, scratch_i, data_type, flag_compute_index_sort);
    ...
    if(flag_compute_index == SORT_COMPUTE_RANK && n_data > 0) {
        merge_sort(idx_sort, n_data, index, SID_SIZE_T, SORT_COMPUTE_INDEX, SORT_COMPUTE_INPLACE);
...
void merge_helper(void *       data,
    ...
    if(right == left + 1)
        ...
        merge_helper(data, index, left, left + midpoint_distance, scratch_d, scratch_i, data_type, flag_compute_index);
        merge_helper(data, index, left + midpoint_distance, right, scratch_d, scratch_i, data_type, flag_compute_index);
