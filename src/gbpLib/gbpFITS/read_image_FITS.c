...
int read_image_FITS(void **image, SID_Datatype *dtype, int *n_D, int **D, const char *filename) {
    ...
    if(status) {
        ffgmsg(error_msg);
        SID_exit_error("FITS open file: filename={%s} status=%d message={%s}", SID_ERROR_IO_OPEN, filename, status,
                       error_msg);
    }
    ...
    if(status) {
        ffgmsg(error_msg);
        SID_exit_error("FITS read {%s}: filename={%s} status=%d message={%s}", SID_ERROR_IO_OPEN, keyname, filename,
                       status, error_msg);
    }
    ...
    switch(dtype_fits) {
        case 16:
            ...
            if(status) {
                ffgmsg(error_msg);
                SID_exit_error("FITS image: filename={%s} dtype_fits=%d status=%d message={%s}", SID_ERROR_IO_OPEN,
                               filename, dtype_fits, status, error_msg);
            }
        ...
        case -64:
            ...
            break;
        default:
            SID_exit_error("Unsupported datatype {%d} in write_image_FITS", SID_ERROR_LOGIC, dtype_fits);
            break;
    }
