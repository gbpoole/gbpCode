...
void autotune_MCMC_temperature(MCMC_info *MCMC) {
    ...
    SID_log("Autotuning the temperature (target=%6.2lf%%, threshold=%6.2lf%%)...",
            SID_LOG_OPEN | SID_LOG_TIMER,
            MCMC->success_target,
            MCMC->success_threshold);
    ...
    while(fabs(success - success_target) > success_threshold &&
          ((temperature_hi - temperature_lo) / temperature) > MCMC_AUTOTUNE_CONVERGENCE_THRESH) {
        // Perform bisection
        if(i_autotune > 0) {
