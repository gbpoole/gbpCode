...
int main(int argc, char *argv[]) {
    ...
    if((fp_in1 = fopen(filename_in1, "r")) != NULL) {
        SID_fread_verify(&n_iterations_file_total, sizeof(int), 1, fp_in1);
        SID_fread_verify(&n_iterations_file_burn, sizeof(int), 1, fp_in1);
        SID_fread_verify(&temperature, sizeof(double), 1, fp_in1);
    ...
    if((fp_in2 = fopen(filename_in2, "r")) != NULL) {
        ...
        SID_fread_verify(&n_P, sizeof(int), 1, fp_in2);
        SID_log("n_P=%d", SID_LOG_COMMENT, n_P);
        for(i_P = 0; i_P < n_P; i_P++) {
            for(j_P = 0; j_P < n_P; j_P++) {
                SID_fread_verify(&val, sizeof(double), 1, fp_in1);
                fprintf(fp_out, "%3d %3d %le ", i_P, j_P, val);
                SID_fread_verify(&val, sizeof(double), 1, fp_in2);
                fprintf(fp_out, "%le\n", val);
            }
        }
    } else
