...
int main(int argc, char *argv[]) {
    ...
    flag_write_headers = atoi(argv[6]);
    i_read_step        = 1;

    SID_log("Constructing merger tree matches for snapshots #%d->#%d (n_search=%d)...",
    ...
    if(flag_write_headers)
        mode = WRITE_MATCHES_MODE_TREES | WRITE_MATCHES_CHECK_HEADER;
    else
        mode = WRITE_MATCHES_MODE_TREES;
