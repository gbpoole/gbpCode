...
void write_halo_local(tree_info *trees,
    ...
    fopen_catalog(filename_catalog_root, i_read, read_props_mode, &fp_properties);
    ...
    fprintf(fp_out,
            "%3d %7d %7d %5.2lf %7d %3d %3d %7d %7d %3d %7d %3d %7d %7d %s\n",
            i_read,
            i_halo,
            halo_id,
...
int main(int argc, char *argv[]) {
    ...
    for(i_read = i_read_in; i_read >= GBP_MAX(trees->i_read_start, i_read_in - trees->n_search * trees->i_read_step);
        ...
        sprintf(filename_in,
                "%s/trees/%s/horizontal/trees/horizontal_trees_bridge_forematch_pointers_%03d.dat",
                filename_SSimPL_root,
                filename_trees_root,
                i_read);
        SID_fopen(filename_in, "r", &fp_in_bridge_forematch);
        sprintf(filename_in,
                "%s/trees/%s/horizontal/trees/horizontal_trees_bridge_backmatch_pointers_%03d.dat",
                filename_SSimPL_root,
                filename_trees_root,
                i_read);
