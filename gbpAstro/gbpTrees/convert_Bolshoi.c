        ...
        for(i_y = 0; i_y < 5 && i_write < n_write; i_y++) {
            for(i_z = 0; i_z < 5 && i_write < n_write; i_z++, i_write++) {
                ...
                if(i_write >= i_write_start && i_write <= i_write_stop) {
                    ...
                    for(i_halo = 0; i_halo < n_halos; i_halo++) {
                        if(parent_id_array[i_halo] >= 0) {
                            ...
                            for(; halo_scale_array[halo_id_index[halo_index]] != halo_scale_array[i_halo] &&
                                  halo_id_array[halo_id_index[halo_index]] == parent_id_array[i_halo] && halo_index < n_halos - 1;)
                                halo_index++;
                            halo_index = halo_id_index[halo_index];
                            if(parent_id_array[i_halo] != halo_id_array[halo_index])
                                fprintf(fp_missing_parents_out,
                                        "# Halo %08d parent %08d not found.  Mass= %le Position= %le %le %le \n",
                                        halo_id_array[i_halo],
                                        parent_id_array[i_halo],
                                        M_vir_array[i_halo],
                                        x_array[i_halo],
                                        y_array[i_halo],
                                        z_array[i_halo]);
                    ...
                    for(i_halo = 0; i_halo < n_halos; i_halo++)
                        fprintf(fp_masses_out,
                                "%8d %le %le %le\n",
                                halo_id_array[i_halo],
                                M_vir_in_array[i_halo],
                                M_vir_array[i_halo],
                                1. - M_vir_array[i_halo] / M_vir_in_array[i_halo]);
                    ...
                    for(i_tree = 0, i_halo = 0, i_report = 0; i_tree < n_trees_in; i_tree++) {
                        for(j_halo = 0; j_halo < n_halos_isotree[i_tree]; j_halo++, i_halo++) {
                            ...
                            add_node_to_vertical_tree(trees[j_tree], 0, halo_id, group_id, descendant_id, halo_snap, descendant_snap, &properties);
                            n_halos_tree[j_tree]++;
                        };
