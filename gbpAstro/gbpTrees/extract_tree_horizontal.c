...
int main(int argc, char *argv[]) {
    ...
    if(!strcmp(argv[2], "group") || !strcmp(argv[2], "groups") || !strcmp(argv[2], "GROUP") || !strcmp(argv[2], "GROUPS")) {
    ...
    } else if(!strcmp(argv[2], "subgroup") || !strcmp(argv[2], "subgroups") || !strcmp(argv[2], "SUBGROUP") || !strcmp(argv[2], "SUBGROUPS")) {
    ...
    read_trees_run_parameters(
        filename_root, &i_read_start, &i_read_stop, &i_read_step, &n_search, &flag_fix_bridges, &flag_compute_fragmented, &flag_compute_ghosts);

    // Fetch the final number of trees
    int i_read_last = i_read_stop;
    while(i_read_last >= n_search)
        i_read_last -= n_search;
    ...
    for(i_read = i_read_stop; i_read >= i_read_start; i_read -= i_read_step) {
        ...
        fopen_catalog(filename_catalog_root, i_read, READ_CATALOG_GROUPS | READ_CATALOG_PROPERTIES, &fp_group_properties);
        fopen_catalog(filename_catalog_root, i_read, READ_CATALOG_SUBGROUPS | READ_CATALOG_PROPERTIES, &fp_subgroup_properties);
        ...
        for(i_group = 0, i_subgroup = 0; i_group < n_groups; i_group++) {
            ...
            if(i_tree_extract_group == group_tree_id)
                fprintf(fp_out_groups,
                        "%4d %8d %8d %8d %8d %8d %d %8d %8d %le %le %le\n",
                        i_read,
                        i_group,
                        group_file_index,
                        group_id,
                        n_subgroups_group,
                        group_type,
                        check_if_type_is_fragmented(group_type),
                        subgroup_tree_id_min,
                        subgroup_tree_id_max,
                        group_properties.pos[0],
                        group_properties.pos[1],
                        group_properties.pos[2]);
        }
        SID_fclose(&fp_in);
