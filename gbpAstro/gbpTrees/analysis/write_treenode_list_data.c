    ...
    for(int i_rank = 0; i_rank < SID.n_proc; i_rank++) {
        ...
        if(SID.My_rank == i_rank || SID.I_am_Master) {
            ...
            if(i_rank != SID_MASTER_RANK) {
                SID_Status status;
                SID_Sendrecv(&n_list_local, 1, SID_INT, SID_MASTER_RANK, 1918270, &n_list_i, 1, SID_INT, i_rank,
                             1918270, SID_COMM_WORLD, &status);
            }
            // Allocate buffers
            ...
            while(n_remaining > 0) {
                ...
                if(SID.I_am_Master) {
                    for(int i_buffer = 0; i_buffer < n_buffer_i; i_buffer++, j_list++) {
                        ...
                        for(int i_data = 0; i_data < n_data; i_data++) {
                            if(typ_data[i_data] == SID_INT)
                                fprintf(fp_props_out, " %4d", ((int *)(buffer[i_data]))[i_buffer]);
