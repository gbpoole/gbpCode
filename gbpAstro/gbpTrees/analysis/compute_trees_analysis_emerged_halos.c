...
void compute_trees_analysis_emerged_halos(tree_info *trees, char *filename_out_root_in, int i_type, double logM_min, double dlogM, int n_logM) {
    ...
    int *snap_found;
    init_treenode_info_data(list_halos, SID_FARG snap_found, SID_INT, "Snapshot when found");
    int *snap_lost;
    init_treenode_info_data(list_halos, SID_FARG snap_lost, SID_INT, "Snapshot when lost");
    double *delta_normed;
                ...
                if(n_emerged_i == 0) {
                    ...
                    while(current_progenitor != NULL) {
                        ...
                        if(flag_emerged) {
                            ...
                            delta_normed[i_halos_emerged] =
                                fetch_treenode_delta_t(trees, current_progenitor, current_progenitor->progenitor_first) / t_dyn_z(z, trees->cosmo);
                            delta_emerged[i_halos_emerged] =
                                fetch_treenode_delta_t(trees, current_progenitor, current_progenitor->progenitor_first) / S_PER_GYR;
