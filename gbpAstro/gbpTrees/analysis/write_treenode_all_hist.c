...
void write_treenode_all_hist(tree_info *trees, const char *filename_out_root_in, int i_type, double logM_min, double dlogM, int n_logM) {
    ...
    if(i_type == 1) {
        ...
        SID_Allreduce(
            SID_IN_PLACE, (hist_centrals_Npeak->array), (hist_centrals_Npeak->n_x * hist_centrals_Npeak->n_y), SID_INT, SID_SUM, SID.COMM_WORLD);
        SID_Allreduce(SID_IN_PLACE, (hist_centrals_M->array), (hist_centrals_M->n_x * hist_centrals_M->n_y), SID_INT, SID_SUM, SID.COMM_WORLD);
        SID_Allreduce(
            SID_IN_PLACE, (hist_centrals_Mpeak->array), (hist_centrals_Mpeak->n_x * hist_centrals_Mpeak->n_y), SID_INT, SID_SUM, SID.COMM_WORLD);
        SID_Allreduce(
            SID_IN_PLACE, (hist_substructure_N->array), (hist_substructure_N->n_x * hist_substructure_N->n_y), SID_INT, SID_SUM, SID.COMM_WORLD);
        SID_Allreduce(SID_IN_PLACE,
                      ...
                      SID.COMM_WORLD);
        SID_Allreduce(
            SID_IN_PLACE, (hist_substructure_M->array), (hist_substructure_M->n_x * hist_substructure_M->n_y), SID_INT, SID_SUM, SID.COMM_WORLD);
        SID_Allreduce(SID_IN_PLACE,
