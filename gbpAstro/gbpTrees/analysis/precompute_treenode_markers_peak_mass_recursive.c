    ...
    if(halo != NULL) {
        ...
        if(first_progenitor != NULL) {
            ...
            while(current_progenitor != NULL) {
                ...
                double          M_peak_prog;
                precompute_treenode_markers_peak_mass_recursive(
                    trees, flag_peak_mode_inclusive, markers_array, current_progenitor, &halo_peak_prog, &n_particles_peak_prog, &M_peak_prog);
