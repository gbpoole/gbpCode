...
void write_treenode_list_hist(tree_info *trees, const char *filename_out_root, treenode_list_info *list, double logM_min, double dlogM, int n_logM) {
    ...
    treenode_hist_info *hist_list_Npeak;
    ...
    init_treenode_hist(trees, list->catalog_name, "z", "N_peak", TREENODE_HIST_LOG_Y, &hist_list_Npeak, n_z_bin, logN_min, dlogN, n_logN);
    init_treenode_hist(trees, list->catalog_name, "z", "M", TREENODE_HIST_LOG_Y, &hist_list_M, n_z_bin, logM_min, dlogM, n_logM);
    init_treenode_hist(trees, list->catalog_name, "z", "M_peak", TREENODE_HIST_LOG_Y, &hist_list_Mpeak, n_z_bin, logM_min, dlogM, n_logM);
    for(int i_list = 0; i_list < list->n_list_local; i_list++) {
        ...
        add_to_treenode_hist(trees, hist_list_Npeak, current_halo);
    ...
    free_treenode_hist(&hist_list_Npeak);
