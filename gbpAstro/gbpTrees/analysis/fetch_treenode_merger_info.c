...
int fetch_treenode_merger_info(tree_info *      trees,
    ...
    if((*halo_secondary) != NULL && (*halo_primary) != NULL) {
        ...
        if(!check_treenode_if_group((*halo_secondary))) {
            ...
            flag_success                         = find_treenode_snap_equals_given(
                trees, (*halo_primary), (*halo_secondary)->snap_tree, halo_primary, TREE_PROGENITOR_ORDER_N_PARTICLES_PEAK);
        }
    }
    ...
    if(flag_success) {
        ...
        if(sig_v_p != NULL)
            (*sig_v_p) = fetch_treenode_sigmav(trees, (*halo_primary));
        if(sig_v_s != NULL)
            (*sig_v_s) = fetch_treenode_sigmav(trees, (*halo_secondary));
    } else {
        if(xi != NULL)
            (*xi) = -1;
        if(v_rel != NULL)
