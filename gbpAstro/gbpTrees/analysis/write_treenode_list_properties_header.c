...
void write_treenode_list_properties_header(tree_info *trees, treenode_list_info *list, FILE *fp_props_out) {
    ...
    int n_properties = 0;
    while(write_treenode_list_properties_set_ith(trees, n_properties, NULL, NULL, NULL, NULL, NULL))
        n_properties++;
    ...
    if(SID.I_am_Master) {
        ...
        for(int i_write = 0; i_write < n_properties; i_write++) {
            char write_name[128];
            write_treenode_list_properties_set_ith(trees, i_write, NULL, write_name, NULL, NULL, NULL);
            if(i_write == 0) {
                if(flag_groups_list)
