    ...
    if(halo != NULL) {
        ...
        if(halo->parent_top == NULL) {
            if(trees->group_properties != NULL) {
                ...
                SID_exit_error(
                        "Group properties are not defined in fetch_treenode_n_particles().  They probably have not been read.",
                        SID_ERROR_LOGIC);
        } else {
            if(trees->subgroup_properties != NULL) {
                ...
                SID_exit_error(
                        "Subgroup properties are not defined in fetch_treenode_n_particles().  They probably have not been read.",
                        SID_ERROR_LOGIC);
        }
        return (n_particles);
    }
