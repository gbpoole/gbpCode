    ...
    if(halo != NULL) {
        if(halo->parent_top == NULL) {
            if(trees->group_backmatch_pointers != NULL)
                ...
                SID_exit_error("Group backmatch pointers are not defined.  They probably have not been read.",
                               SID_ERROR_LOGIC);
        } else {
            if(trees->subgroup_backmatch_pointers != NULL)
                return (trees->subgroup_backmatch_pointers[halo->snap_tree][halo->neighbour_index]);
            else
                SID_exit_error("Subgroup backmatch pointers are not defined.  They probably have not been read.",
                               SID_ERROR_LOGIC);
        }
    }
    return (NULL);
