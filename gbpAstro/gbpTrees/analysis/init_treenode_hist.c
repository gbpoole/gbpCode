...
void init_treenode_hist(tree_info *          trees,
    ...
    for(i_prop = 0; i_prop < TREENODE_HIST_N_PROPS; i_prop++) {
        if(!strcmp(x_name_in, treenode_hist_props.name[i_prop])) {
            ...
            if(n_args_x > TREENODE_HIST_N_ARGS_MAX)
                SID_exit_error("Increase TREENODE_HIST_N_ARGS_MAX to at least %d.", SID_ERROR_LOGIC, n_args_x);
        }
        if(!strcmp(y_name_in, treenode_hist_props.name[i_prop])) {
            ...
            if(n_args_y > TREENODE_HIST_N_ARGS_MAX)
                SID_exit_error("Increase TREENODE_HIST_N_ARGS_MAX to at least %d.", SID_ERROR_LOGIC, n_args_y);
        }
    }

    // Sanity check
    if(!flag_x_found)
        SID_exit_error("Could not find property definition for {%s} in init_treenode_hist.", SID_ERROR_LOGIC, x_name_in);
    if(!flag_y_found)
        SID_exit_error("Could not find property definition for {%s} in init_treenode_hist.", SID_ERROR_LOGIC, y_name_in);
    ...
    for(int i_arg = 0; i_arg < n_args_x; i_arg++) {
        if(treenode_hist_props.arg_type[i_prop_x][i_arg] == SID_DOUBLE)
            ...
            SID_exit_error("Invalid datatype specified in init_treenode_hist for x-axis.", SID_ERROR_LOGIC);
    }
    for(int i_arg = 0; i_arg < n_args_y; i_arg++) {
        if(treenode_hist_props.arg_type[i_prop_y][i_arg] == SID_DOUBLE)
            ...
            SID_exit_error("Invalid datatype specified in init_treenode_hist for y-axis.", SID_ERROR_LOGIC);
    }
