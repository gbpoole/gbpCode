...
void read_trees(const char *filename_SSimPL_root,
    ...
    SID_log("Reading merger trees for snapshots #%d->#%d (step=%d)...",
    ...
    int               flag_read_extended_pointers         = GBP_FALSE;
    ...
    if(check_mode_for_flag(read_mode, TREE_READ_EXTENDED_POINTERS)) {
        ...
        init_trees_data(
            (*trees), (void ***)(&descendant_pointers_subgroups_score), 0, INIT_TREE_DATA_SUBGROUPS, "descendant_pointers_subgroups_score");
        init_trees_data(
            (*trees), (void ***)(&progenitor_pointers_subgroups_score), 0, INIT_TREE_DATA_SUBGROUPS, "progenitor_pointers_subgroups_score");
        ...
        flag_read_extended_pointers           = GBP_TRUE;
    }
    ...
    for(; i_read >= i_read_start; i_read -= i_read_step,
        ...
        for(; i_group < n_groups; i_group++) {
            ...
            SID_fread_all_buffer(&n_particles_group, sizeof(int), 1, fp_groups_in_buffer);
            int group_id;
            SID_fread_all_buffer(&group_id, sizeof(int), 1, fp_trees_in_buffer);
            int group_tree_case;
            SID_fread_all_buffer(&group_tree_case, sizeof(int), 1, fp_trees_in_buffer);
            int group_descendant_id;
            SID_fread_all_buffer(&group_descendant_id, sizeof(int), 1, fp_trees_in_buffer);
            int group_tree_id;
            SID_fread_all_buffer(&group_tree_id, sizeof(int), 1, fp_trees_in_buffer);
            int group_file_offset;
            SID_fread_all_buffer(&group_file_offset, sizeof(int), 1, fp_trees_in_buffer);
            int group_file_index;
            SID_fread_all_buffer(&group_file_index, sizeof(int), 1, fp_trees_in_buffer);
            int group_n_particles_peak;
            SID_fread_all_buffer(&group_n_particles_peak, sizeof(int), 1, fp_trees_in_buffer);
            int n_subgroups_group;
            SID_fread_all_buffer(&n_subgroups_group, sizeof(int), 1, fp_trees_in_buffer);
            ...
            if(flag_read_sub_pointers && n_subgroups_group > n_hierarchy_alloc) {
                ...
                n_hierarchy_alloc           = GBP_MAX(2 * n_hierarchy_alloc, n_subgroups_group);
                group_substructure_pointers = (tree_node_info **)SID_malloc(n_hierarchy_alloc * sizeof(tree_node_info *));
            }
            ...
            for(int j_subgroup = 0; j_subgroup < n_subgroups_group; j_subgroup++) {
                ...
                SID_fread_all_buffer(&n_particles_subgroup, sizeof(int), 1, fp_subgroups_in_buffer);
                int subgroup_id;
                SID_fread_all_buffer(&subgroup_id, sizeof(int), 1, fp_trees_in_buffer);
                int subgroup_tree_case;
                SID_fread_all_buffer(&subgroup_tree_case, sizeof(int), 1, fp_trees_in_buffer);
                int subgroup_descendant_id;
                SID_fread_all_buffer(&subgroup_descendant_id, sizeof(int), 1, fp_trees_in_buffer);
                int subgroup_tree_id;
                SID_fread_all_buffer(&subgroup_tree_id, sizeof(int), 1, fp_trees_in_buffer);
                int subgroup_file_offset;
                SID_fread_all_buffer(&subgroup_file_offset, sizeof(int), 1, fp_trees_in_buffer);
                int subgroup_file_index;
                SID_fread_all_buffer(&subgroup_file_index, sizeof(int), 1, fp_trees_in_buffer);
                int subgroup_n_particles_peak;
                SID_fread_all_buffer(&subgroup_n_particles_peak, sizeof(int), 1, fp_trees_in_buffer);
            ...
            if(flag_read_sub_pointers) {
                ...
                int flag_central_set = GBP_FALSE;
                for(int j_subgroup = 0; j_subgroup < n_subgroups_group; j_subgroup++) {
        ...
        if(flag_read_extended_pointers) {
            ...
            if(i_file_bridge >= 0 && i_file_bridge < n_snaps)
                read_trees_pointers(
                    (*trees), filename_input_dir_horizontal_trees, i_file_bridge, i_read_bridge, READ_TREES_POINTERS_BRIDGE_FOREMATCH);
            // Read backmatch pointers
            if(i_file_backmatch >= 0 && i_file_backmatch < n_snaps)
                read_trees_pointers(
                    (*trees), filename_input_dir_horizontal_trees, i_file_backmatch, i_read_backmatch, READ_TREES_POINTERS_BRIDGE_BACKMATCH);
        }
