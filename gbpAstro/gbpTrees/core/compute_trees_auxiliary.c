...
void compute_trees_auxiliary(char *filename_root,
    ...
    SID_log("Constructing auxiliary tree files for snapshots #%d->#%d (step=%d)...",
        ...
        for(i_write = 0; i_write < n_write; i_write++) {
            ...
            for(j_tree = 0, i_tree = 0; j_tree < n_trees; j_tree++) {
                for(i_halo = 0; i_halo < n_halos_tree[j_tree]; i_halo++, i_tree++) {
                    ...
                    if(i_snap < 0 || i_snap >= n_snap)
                        SID_exit_error("i_snap=%d! (%d)", SID_ERROR_LOGIC, i_snap, n_snap);
                    n_halos_snap[i_snap]++;
                }
            }
            ...
            for(j_tree = 0; j_tree < n_trees; j_tree++) {
                for(i_halo = 0; i_halo < n_halos_tree[j_tree]; i_halo++) {
                    ...
                    if(i_snap < 0 || i_snap >= n_snap)
                        SID_exit_error("i_snap=%d! (should be 0->%d)", SID_ERROR_LOGIC, i_snap, n_snap - 1);
                    if(j_halo < 0 || j_halo >= n_halos)
                        SID_exit_error("j_halo=%d! (shoule be 0->%d)", SID_ERROR_LOGIC, j_halo, n_halos - 1);
            ...
            for(i_snap = 0, n_particles = 0, n_ids = 0, offset_snap_tree = 0; i_snap < n_snap; i_snap++) {
                SID_log("Processing snapshot #%d for auxilliary %sgroup tree file #%d...",
                        SID_LOG_OPEN | SID_LOG_TIMER,
                        i_read_stop - (n_snap - i_snap - 1) * i_read_step,
                        group_text_prefix,
                        i_write + 1);
