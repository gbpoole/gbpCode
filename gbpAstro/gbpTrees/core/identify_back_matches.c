...
void identify_back_matches(tree_horizontal_info **halos,
    ...
    for(j_file_1 = i_file + 1, j_file_2 = i_file, j_read_1 = i_read + i_read_step, j_read_2 = i_read, i_search = 0;
        j_read_1 <= i_read_stop && i_search < n_search;
        j_file_1++, j_read_1 += i_read_step, i_search++) {
    ...
    for(i_halo = 0; i_halo < n_halos_2_matches; i_halo++) {
        ...
        if(halo_i->forematch_first.halo != NULL) {
            ...
            for(k_halo = 0; k_halo < halo_i->n_back_matches; k_halo++) {
                if(halo_i->forematch_first.halo == back_matches[k_halo].halo)
                    break;
            }
        ...
        for(j_halo = 0, l_halo = 0; j_halo < n_list; j_halo++) {
            ...
            if(backmatch_keep[j_halo_sorted]) {
                ...
                if(back_match->flag_two_way || back_match->flag_back_match) {
                    halos[(back_match->halo->file) % n_wrap][back_match->halo->index].bridge_backmatch.halo            = &(halos_i[i_halo]);
                    halos[(back_match->halo->file) % n_wrap][back_match->halo->index].bridge_backmatch.score           = back_match->score;
                    halos[(back_match->halo->file) % n_wrap][back_match->halo->index].bridge_backmatch.flag_two_way    = back_match->flag_two_way;
                    halos[(back_match->halo->file) % n_wrap][back_match->halo->index].bridge_backmatch.flag_back_match = back_match->flag_back_match;
                }
                l_halo++;
            }
        }
