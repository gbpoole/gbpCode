        ...
        for(int i_type = 0; i_type < 2; i_type++) {
            ...
            if(flag_proceed) {
                ...
                if(n_list_local != n_halos)
                    SID_trap_error(
                        "Count mismatch for %sgroups in read_trees_properties() (ie. %d!=%d)", SID_ERROR_LOGIC, group_text_prefix, n_list_local, n_halos);
                ...
                fopen_catalog(filename_cat_root_in, i_read, open_catalog_mode, &fp_properties);
                    ...
                    if(l_read < n_list_local) {
                        if(k_read == file_idx_list_local[file_idx_list_local_index[l_read]]) {
                            if(SAGE_properties_buffer != NULL)
                                memcpy(&(SAGE_properties_in[nebr_idx_list_local[file_idx_list_local_index[l_read]]]),
                                       SAGE_properties_buffer,
                                       sizeof(halo_properties_SAGE_info));
                            if(SHORT_properties_buffer != NULL)
                                memcpy(&(SHORT_properties_in[nebr_idx_list_local[file_idx_list_local_index[l_read]]]),
                                       SHORT_properties_buffer,
                                       sizeof(halo_properties_SHORT_info));
                            if(properties_buffer != NULL)
                                memcpy(&(properties_in[nebr_idx_list_local[file_idx_list_local_index[l_read]]]),
                                       properties_buffer,
                                       sizeof(halo_properties_info));
                            if(profiles_buffer != NULL)
                                memcpy(&(profiles_in[nebr_idx_list_local[file_idx_list_local_index[l_read]]]),
                                       profiles_buffer,
                                       sizeof(halo_profile_info));
                ...
                if(l_read != n_list_local)
                    SID_trap_error("An incorrect number of matches were read (ie. %d!=%d) for snaphot %d", SID_ERROR_LOGIC, l_read, n_list_local, i_read);
                for(i_neighbour = 0; i_neighbour < n_halos; i_neighbour++)
