...
void compute_forests(char *filename_root_out, int n_search_forests) {
    ...
    if(SID.I_am_Master) {
        ...
        for(i_read = i_read_stop, n_snap = 0; i_read >= i_read_start; i_read -= i_read_step)
            n_snap++;
        ...
        for(i_read = i_read_stop, n_halos_groups = 0, n_halos_subgroups = 0, j_read = 0; i_read >= i_read_start;
        ...
        for(i_forest = 0; i_tree < n_trees_group; i_forest++) {
            ...
            if(n_trees_forest_groups[i_forest] <= 0)
                SID_exit_error("A group forest (%d) has been assigned no trees.", SID_ERROR_LOGIC, i_forest);
        }
        ...
        merge_sort(
            subgroup_forest_array, (size_t)n_trees_subgroup, &subgroup_forest_array_index, SID_INT, SORT_COMPUTE_INDEX, SORT_COMPUTE_NOT_INPLACE);
        i_tree = 0;
        ...
        for(i_forest = 0; i_tree < n_trees_subgroup; i_forest++) {
            ...
            if(n_trees_forest_subgroups[i_forest] <= 0)
                SID_exit_error("A subgroup forest (%d) has been assigned no trees.", SID_ERROR_LOGIC, i_forest);
        }
        ...
        SID_log("Determining contemporaneous halo counts...", SID_LOG_OPEN | SID_LOG_TIMER);
        ...
        for(i_read = i_read_stop, j_read = 0; i_read >= i_read_start; i_read -= i_read_step, j_read += i_read_step) {
            SID_log("Processing snapshot #%03d...", SID_LOG_OPEN | SID_LOG_TIMER, i_read);
            sprintf(filename_in, "%s/horizontal_trees_%03d.dat", filename_output_dir_horizontal_trees, i_read);
            SID_fopen(filename_in, "r", &fp_in);
            SID_fread_all(&n_step_in, sizeof(int), 1, &fp_in);
        ...
        for(i_tree = 0; i_tree < n_trees_group; i_tree++)
            fprintf(fp_out, "%d %d %d %d\n", i_tree, group_forest_array[i_tree], n_halos_tree_group[i_tree], n_groups_snap_tree_max[i_tree]);
        fclose(fp_out);
        ...
        fprintf(fp_out, "#        (%02d): Max No. of contemporaneous halos in tree\n", i_column++);
        for(i_tree = 0; i_tree < n_trees_subgroup; i_tree++)
            fprintf(fp_out, "%d %d %d %d\n", i_tree, subgroup_forest_array[i_tree], n_halos_tree_subgroup[i_tree], n_subgroups_snap_tree_max[i_tree]);
        fclose(fp_out);
