...
void identify_progenitors(tree_horizontal_info **halos,
    ...
    int i_search = 0;
    for(; j_read_2 <= i_read_stop && i_search < n_search; j_file_2++, j_read_2 += i_read_step, i_search++) {
        ...
        if(flag_fix_bridges) {
                                ...
                                for(int k_halo = 0; k_halo < n_back_matches && !flag_unchecked; k_halo++) {
                                    ...
                                    if(current_back_match->file == j_file_2) {
                                        if(current_back_match->index == match_id[i_halo]) {
                                            ...
                                            //    though, because we want to continue checking emerged candidates in the descendant line of the new
                                            //    match
            ...
            for(int i_halo = 0; i_halo < (*n_halos_1_matches); i_halo++) {
                ...
                if(halo_j != NULL) {
                    ...
                    if(halo_j->forematch_best.halo == NULL && halo_i->forematch_first.halo != halo_i->forematch_default.halo &&
