...
void read_forests(const char *filename_root_in,
    ...
    merge_sort(
        (*tree2forest_mapping_group), (size_t)n_trees_group, &tree2forest_mapping_group_index, SID_INT, SORT_COMPUTE_INDEX, SORT_COMPUTE_NOT_INPLACE);
    ...
    while((*tree2forest_mapping_group)[tree2forest_mapping_group_index[i_tree]] < 0 && i_tree < (n_trees_group - 1))
        i_tree++;
    if((*tree2forest_mapping_group)[tree2forest_mapping_group_index[i_tree]] < 0)
        i_tree++;
    for(i_forest = 0; i_tree < n_trees_group; i_forest++) {
        ...
        while((*tree2forest_mapping_group)[tree2forest_mapping_group_index[i_tree]] == i_forest && i_tree < (n_trees_group - 1))
            i_tree++;
        if((*tree2forest_mapping_group)[tree2forest_mapping_group_index[i_tree]] == i_forest)
            i_tree++;
    ...
    merge_sort((*tree2forest_mapping_subgroup),
               (size_t)n_trees_subgroup,
               &tree2forest_mapping_subgroup_index,
               SID_INT,
               SORT_COMPUTE_INDEX,
    ...
    while((*tree2forest_mapping_subgroup)[tree2forest_mapping_subgroup_index[i_tree]] < 0 && i_tree < (n_trees_subgroup - 1))
        i_tree++;
    if((*tree2forest_mapping_subgroup)[tree2forest_mapping_subgroup_index[i_tree]] < 0)
        i_tree++;
    for(i_forest = 0; i_tree < n_trees_subgroup; i_forest++) {
        ...
        while((*tree2forest_mapping_subgroup)[tree2forest_mapping_subgroup_index[i_tree]] == i_forest && i_tree < (n_trees_subgroup - 1))
            i_tree++;
        if((*tree2forest_mapping_subgroup)[tree2forest_mapping_subgroup_index[i_tree]] == i_forest)
            i_tree++;
        n_trees_forest = i_tree - i_tree_start;
    ...
    SID_log("No. of trees in largest group    forest = %d (%d halos)",
            SID_LOG_COMMENT,
            (*n_trees_forest_groups_max),
            n_halos_forest_group[tree2forest_mapping_group_largest]);
    SID_log("No. of trees in largest subgroup forest = %d (%d halos)",
            SID_LOG_COMMENT,
    ...
    split_forests_n_ways(n_halos_forest_group, (*n_forests_group), SID.n_proc, &tree_count_split, &forest_lo_split, &forest_hi_split);
    ...
    split_forests_n_ways(n_halos_forest_subgroup, (*n_forests_subgroup), SID.n_proc, &tree_count_split, &forest_lo_split, &forest_hi_split);
    ...
    if(SID.n_proc > 1) {
        ...
        for(i_rank = 0; i_rank < SID.n_proc; i_rank++) {
            ...
            SID_Bcast(&forest_lo_rank, 1, SID_INT, i_rank, SID_COMM_WORLD);
            SID_Bcast(&forest_hi_rank, 1, SID_INT, i_rank, SID_COMM_WORLD);
            SID_Bcast(&tree_count_rank, 1, SID_INT, i_rank, SID_COMM_WORLD);
            SID_Bcast(&n_halos_rank, 1, SID_INT, i_rank, SID_COMM_WORLD);
            SID_Bcast(&n_alloc_rank, 1, SID_INT, i_rank, SID_COMM_WORLD);
            SID_log("Rank #%4d will process %5d    group forests (%5d trees, %8d halos in total; alloc for %8d halos)",
                    SID_LOG_COMMENT,
                    i_rank,
                    forest_hi_rank - forest_lo_rank + 1,
                    tree_count_rank,
                    n_halos_rank,
                    n_alloc_rank);
        }
        for(i_rank = 0; i_rank < SID.n_proc; i_rank++) {
            ...
            SID_Bcast(&n_halos_rank, 1, SID_INT, i_rank, SID_COMM_WORLD);
            SID_Bcast(&n_alloc_rank, 1, SID_INT, i_rank, SID_COMM_WORLD);
            SID_log("Rank #%4d will process %5d subgroup forests (%5d trees, %8d halos in total; alloc for %8d halos)",
                    SID_LOG_COMMENT,
                    i_rank,
                    forest_hi_rank - forest_lo_rank + 1,
                    tree_count_rank,
