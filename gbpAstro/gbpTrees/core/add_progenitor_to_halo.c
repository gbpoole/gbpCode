...
void add_progenitor_to_halo(tree_horizontal_info **halos,
    ...
    if(halos_j[j_halo].n_progenitors == 1) {
        ...
        // ... inherit root file number ...
        new_progenitor.halo->file_root = halos_j[j_halo].file_root;
    ...
    // ... else add a next progenitor ...
        ...
        if(file_offset_new <= 0 || file_offset_old <= 0)
            SID_trap_error("Progenitor file offset is invalid (ie either %d<=0 or %d<=0).", SID_ERROR_LOGIC, file_offset_new, file_offset_old);
        ...
        if(check_if_halo_is_better_main_progenitor(&(halos_j[j_halo]), &old_progenitor, &new_progenitor)) {
            ...
            // ... and give the old main progenitor the new id and root file number...
            change_horizontal_ID_recursive(old_progenitor.halo, i_file, old_progenitor.halo->id, (*max_id)++);
