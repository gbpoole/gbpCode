...
#define F_MATCH_MOMENT_DIFF_MIN_DEFAULT -0.2
...
#define TREE_SUBSTRUCTURE_ORDER_DEFAULT SID_TTTP00
#define TREE_PROGENITOR_ORDER_DELUCIA SID_TTTP01
#define TREE_PROGENITOR_ORDER_N_PARTICLES SID_TTTP02
#define TREE_PROGENITOR_ORDER_N_PARTICLES_INCLUSIVE SID_TTTP03
#define TREE_PROGENITOR_ORDER_N_PARTICLES_PEAK SID_TTTP04
#define TREE_PROGENITOR_ORDER_N_PARTICLES_INCLUSIVE_PEAK SID_TTTP05
#define TREE_READ_EXTENDED_POINTERS SID_TTTP06
#define TREE_READ_HEADER_ONLY SID_TTTP07
#define TREE_MODE_REFERENCE SID_TTTP08
#define TREE_MODE_SUBSTRUCTURE_HIERARCHY_ON SID_TTTP09
...
int tree_case_flag_list[] = {
    TREE_CASE_NO_PROGENITORS,     TREE_CASE_STRAYED,          TREE_CASE_MERGER,     TREE_CASE_FRAGMENTED_NEW,   TREE_CASE_FRAGMENTED_STRAYED,
    TREE_CASE_FRAGMENTED_NORMAL,  TREE_CASE_FRAGMENTED_OTHER, TREE_CASE_2WAY_MATCH, TREE_CASE_MOST_MASSIVE,     TREE_CASE_DOMINANT,
    TREE_CASE_DOMINANT_DROPPED,   TREE_CASE_BRIDGED,          TREE_CASE_DROPPED,    TREE_CASE_EMERGED,          TREE_CASE_EMERGED_CANDIDATE,
    TREE_CASE_MATCHED_TO_EMERGED, TREE_CASE_MERGER_PRIMARY,   TREE_CASE_REMNANT,    TREE_CASE_SET_BY_BACKMATCH, TREE_CASE_GHOST,
    TREE_CASE_GHOST_NULL,         TREE_CASE_UNPROCESSED,      TREE_CASE_INVALID};
const char *tree_case_flag_list_text[] = {"NO_PROGENITORS",     "STRAYED",          "MERGER",  "FRAGMENTED_NEW",   "FRAGMENTED_STRAYED",
                                          "FRAGMENTED_NORMAL",  "FRAGMENTED_OTHER", "2WAY",    "MOST_MASSIVE",     "DOMINANT",
                                          "DOMINANT_DROPPED",   "BRIDGED",          "DROPPED", "EMERGED",          "EMERGED_CANDIDATE",
                                          "MATCHED_TO_EMERGED", "MERGER_PRIMARY",   "REMNANT", "SET_BY_BACKMATCH", "GHOST",
                                          "GHOST_NULL",         "UNPROCESSED",      "INVALID"};
#else
...
#define READ_TREES_CATALOGS_SUBGROUPS SID_TTTP01
#define READ_TREES_CATALOGS_GROUPS SID_TTTP02
#define READ_TREES_CATALOGS_PROFILES SID_TTTP03
#define READ_TREES_CATALOGS_SAGE SID_TTTP04
#define READ_TREES_CATALOGS_SHORT SID_TTTP05
...
struct tree_info {
    ...
    char filename_root_horizontal[SID_MAX_FILENAME_LENGTH];
    char filename_root_horizontal_trees[SID_MAX_FILENAME_LENGTH];
    char filename_root_analysis[SID_MAX_FILENAME_LENGTH];
    char filename_SSimPL_dir[SID_MAX_FILENAME_LENGTH];
    char filename_halos_version[SID_MAX_FILENAME_LENGTH];
    char filename_trees_version[SID_MAX_FILENAME_LENGTH];
    char name[SID_MAX_FILENAME_LENGTH];
    // Snapshot info
    int     i_read_start;
    int     i_read_stop;
    int     i_read_step;
...
void read_trees(const char *filename_SSiMPL_root,
                ...
                int         read_mode,
                tree_info **trees);
void read_trees_data(tree_info *trees, char *filename_root, int mode, SID_Datatype data_type, const char *name);
void init_trees_data(tree_info *trees, void ***rval, size_t data_size, int mode, const char *name, ...);
void init_trees_lookup(tree_info *trees);
void update_trees_lookup(tree_info *trees, int i_file);
int  find_tree_node(tree_info *trees, int node_file, int node_index, int group_mode, tree_node_info **descendant);
...
void read_trees_pointers(tree_info *trees, const char *filename_input_dir_horizontal_trees, int i_file_ptrs, int i_read_ptrs, int mode);
void read_trees_catalogs(tree_info *trees, int mode);
void read_trees_substructure_hierarchy(tree_info *trees);
void read_AHF_for_trees(char *filename_root, int i_file, plist_info *plist, char *catalog_name, int mode);
...
int  check_if_halo_is_better_main_progenitor(tree_horizontal_info *target_halo, match_info *old_progenitor, match_info *new_progenitor);
...
int  check_if_descendant_is_back_matched(tree_horizontal_info *halo, tree_horizontal_info *halo_to_check);
...
int  check_if_match_is_bigger_peak(tree_horizontal_info *target_halo, match_info *old_match, match_info *new_match);
int  check_if_forced_match_is_better(tree_horizontal_info *target_halo, match_info *old_match, match_info *new_match);
int  check_if_match_is_simple(tree_horizontal_info *target_halo, match_info *match);
int  check_if_match_is_immediate(tree_horizontal_info *target_halo, match_info *match);
...
void check_for_fragmented_halos(int k_match, tree_horizontal_info **groups, int n_groups, int i_write, int j_write, int l_write, int n_wrap);
...
void identify_back_matches(tree_horizontal_info **halos,
                           ...
                           double                 f_match_moment_diff_min,
                           int                    flag_match_subgroups);
void identify_bridges(tree_horizontal_info *halos_i, int n_halos_i, int n_search);
void identify_progenitors(tree_horizontal_info **halos,
                          ...
                          double                 f_match_moment_diff_min,
                          int                    flag_match_subgroups);
void finalize_bridged_halo_list(tree_horizontal_info *halos_i, int n_halos_i, int i_file, int n_search, int n_files);
void set_largest_descendants(tree_horizontal_info *halos_i, int n_halos_i);
...
void write_trees_horizontal_log_file(char *                      filename_log,
                                     ...
                                     cosmo_info **               cosmo,
                                     int                         flag_init);
void write_trees_horizontal_report(int n_halos_i, int n_halos_max, tree_horizontal_info *halos_i);
...
void read_trees_final_totals(char *filename_output_dir_horizontal_trees,
                             ...
                             int * n_trees_subgroup,
                             int * n_trees_group);
void split_forests_n_ways(int *n_halos_forest, int n_forests, int n_split, int **tree_count_split, int **forest_lo_split, int **forest_hi_split);
int  read_matches_header(char *filename_root_in,
                         ...
                         int * n_groups_max,
                         int * n_halos_max);
void read_trees_horizontal(void **groups,
                           int *  n_groups_in,
                           void **subgroups,
                           int *  n_subgroups_in,
                           int *  n_subgroups_group,
...
void propagate_progenitor_info(int *        n_groups,
                               ...
                               char *       filename_output_dir,
                               int          flag_compute_fragmented);
void propagate_dominant_substructures(tree_horizontal_extended_info **groups,
                                      int *                           n_groups,
                                      ...
                                      int                             i_read_step,
                                      int                             n_wrap);
void propagate_n_particles_peak(tree_horizontal_extended_info **groups,
                                int *                           n_groups,
                                tree_horizontal_extended_info **subgroups,
                                int *                           n_subgroups,
                                int **                          n_subgroups_group,
                                ...
                                int                             i_read_step,
                                int                             n_wrap);
void propagate_bridge_info(tree_horizontal_extended_info **groups,
                           int *                           n_groups,
                           tree_horizontal_extended_info **subgroups,
                           int *                           n_subgroups,
                           int **                          n_subgroups_group,
...
void propagate_fragmented_info(tree_horizontal_extended_info **groups,
                               ...
                               int                             i_read_step,
                               int                             n_wrap);
void propagate_fix_fragmented_info(tree_horizontal_extended_info **groups,
                                   int *                           n_groups,
                                   tree_horizontal_extended_info **subgroups,
                                   int *                           n_subgroups,
                                   int **                          n_subgroups_group,
                                   ...
                                   int                             i_read_step,
                                   int                             n_wrap);
void propagate_merger_info(tree_horizontal_extended_info **groups,
                           int *                           n_groups,
                           tree_horizontal_extended_info **subgroups,
                           int *                           n_subgroups,
                           int **                          n_subgroups_group,
...
void finalize_trees(tree_info *trees, int group_progenitor_mode, int subgroup_progenitor_mode);
...
void write_tree_run_parameters(char * filename_root_out,
                               ...
                               int    flag_compute_fragmented,
                               int    flag_compute_ghosts);
void write_trees_vertical(tree_info *trees, int grid_size, const char *filename_root_out);
...
int  add_node_to_substructure_hierarchy(tree_info *trees, tree_node_info *subhalo, tree_node_info *parent);
...
void compute_peak_particle_count_recursive(tree_info *     trees,
                                           tree_node_info *this_halo,
                                           int *           flag_fragmented,
                                           int *           n_particles_peak,
                                           int *           n_particles_inclusive_peak);
