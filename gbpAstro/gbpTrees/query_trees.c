...
int main(int argc, char *argv[]) {
    ...
    if(argc == 6) {
        ...
        if(i_read != trees->snap_list[i_file])
            SID_exit_error("Invalid snapshot specified {%d}.", SID_ERROR_LOGIC, i_read);
    ...
    for(; i_read <= trees->i_read_stop && ((find_mode == 1 && i_halo >= 0) || find_mode == 0);
        ...
        if(find_mode == 1 && !flag_done)
            SID_exit_error("Halo {%d} could not be processed.", SID_ERROR_LOGIC, i_halo);
        ...
        if(flag_done) {
            ...
            fprintf(fp_out,
                    "%le %7.3lf %3d %7d %7d %5d %5d %5.2lf %5.2lf %5.2lf %5.2lf %7d %7d %s\n",
                    trees->a_list[i_file],
                    trees->z_list[i_file],
                    i_read,
                    i_halo,
                    ...
                    halo_type_string);
            fprintf(fp_out_ptrs,
                    "%le %7.3lf %3d %7d %7d %3d %6d %6d %6d %6d %6d %6d %6d %6d %6d %6d %6d\n",
                    trees->a_list[i_file],
                    trees->z_list[i_file],
                    i_read,
                    i_halo,
