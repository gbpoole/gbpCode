...
int main(int argc, char *argv[]) {
    ...
    for(snap_i = snap_stop, i_snap = 0; snap_i >= snap_start; snap_i -= snap_step, i_snap++) {
        ...
        if(tree_type == 1) {
            ...
            SID_fread_verify(&i_file_group_ghosts, sizeof(int), 1, fp_in);
            SID_fread_verify(&n_file_group_ghosts, sizeof(int), 1, fp_in);
            SID_fread_verify(&n_group_ghosts, sizeof(int), 1, fp_in);
            fclose(fp_in);
            ...
            if((fp_in = fopen(filename, "r")) == NULL)
                SID_exit_error("Could not open file {%s}", SID_ERROR_IO_OPEN, filename);
            SID_fread_verify(&i_file_subgroup_ghosts, sizeof(int), 1, fp_in);
            SID_fread_verify(&n_file_subgroup_ghosts, sizeof(int), 1, fp_in);
            SID_fread_verify(&n_subgroup_ghosts, sizeof(int), 1, fp_in);
            fclose(fp_in);
        }
        ...
        int n_trees_group;
        SID_fread_verify(&n_step, sizeof(int), 1, fp_in);
        SID_fread_verify(&n_search, sizeof(int), 1, fp_in);
        SID_fread_verify(&n_groups, sizeof(int), 1, fp_in);
        SID_fread_verify(&n_subgroups, sizeof(int), 1, fp_in);
        SID_fread_verify(&n_groups_max, sizeof(int), 1, fp_in);
        SID_fread_verify(&n_subgroups_max, sizeof(int), 1, fp_in);
        SID_fread_verify(&n_trees_subgroup, sizeof(int), 1, fp_in);
        SID_fread_verify(&n_trees_group, sizeof(int), 1, fp_in);
        ...
        for(i_group = 0, i_subgroup = 0; i_group < n_groups; i_group++) {
            ...
            SID_fread_verify(&group_id, sizeof(int), 1, fp_in);
            SID_fread_verify(&group_type, sizeof(int), 1, fp_in);
            SID_fread_verify(&group_descendant_id, sizeof(int), 1, fp_in);
            SID_fread_verify(&group_tree_id, sizeof(int), 1, fp_in);
            if(tree_type == 0)
                SID_fread_verify(&group_file_offset, sizeof(int), 1, fp_in);
            else
                group_file_offset = 1;
            SID_fread_verify(&group_index, sizeof(int), 1, fp_in);
            SID_fread_verify(&n_subgroups_group, sizeof(int), 1, fp_in);
            ...
            for(j_subgroup = 0; j_subgroup < n_subgroups_group; i_subgroup++, j_subgroup++) {
                ...
                int subgroup_index;
                SID_fread_verify(&subgroup_id, sizeof(int), 1, fp_in);
                SID_fread_verify(&subgroup_type, sizeof(int), 1, fp_in);
                SID_fread_verify(&subgroup_descendant_id, sizeof(int), 1, fp_in);
                SID_fread_verify(&subgroup_tree_id, sizeof(int), 1, fp_in);
                if(tree_type == 0)
                    SID_fread_verify(&subgroup_file_offset, sizeof(int), 1, fp_in);
                else
                    subgroup_file_offset = 1;
                SID_fread_verify(&subgroup_index, sizeof(int), 1, fp_in);
