...
int main(int argc, char *argv[]) {
    ...
    SID_log("Computing match score from column #%d in {%s} to column #%d in {%s}...",
    ...
    for(int i_test = 1; i_test < 5000000; i_test *= 5)
        fprintf(fp_test, "%6d %le\n", i_test, maximum_match_score((double)i_test));
    fclose(fp_test);
    ...
    for(int i_from = 0, i_to = 0; i_from < n_ids_from; i_from++) {
        ...
        while(ids_to[ids_to_index[i_to]] < ids_from[idx_from] && i_to < (n_ids_to - 1))
            i_to++;
        if(ids_to[ids_to_index[i_to]] == ids_from[idx_from])
            score += pow((double)(idx_from + 1), MATCH_SCORE_RANK_INDEX);
    }
    SID_log("Score = %le", SID_LOG_COMMENT, score);
