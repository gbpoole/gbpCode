...
int main(int argc, char *argv[]) {
    ...
    if(!strcpy(prefix_text, "subgroup") || !strcpy(prefix_text, "subgroups") || !strcpy(prefix_text, "sub")) {
        sprintf(prefix_text, "sub");
        catalog_read_mode = READ_CATALOG_SUBGROUPS | READ_CATALOG_PROPERTIES;
        matches_read_mode = MATCH_SUBGROUPS;
    } else if(!strcpy(prefix_text, "group") || !strcpy(prefix_text, "groups")) {
        sprintf(prefix_text, "");
    ...
    for(i_halo = 0, j_halo = 0; j_halo < n_halos_j && i_halo < n_halos_i; j_halo++) {
        while(match_ids[match_index[i_halo]] < j_halo && i_halo < (n_halos_i - 1))
            i_halo++;
    ...
    fopen_catalog(filename_catalog_root, i_read, catalog_read_mode, &fp_properties_i);
    fopen_catalog(filename_catalog_root, j_read, catalog_read_mode, &fp_properties_j);
    ...
    fprintf(fp_out,
            "# Catalog for matches {root %s} and catalog {root %s}; snap No. %d to %d.\n",
    ...
    for(i_halo = 0; i_halo < n_halos_i; i_halo++) {
        fprintf(fp_out,
                "%10d %10d %10.4le %10.4le %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f\n",
                i_halo,
                match_ids[i_halo],
