    ...
    for(i_snap = start_snap; i_snap <= stop_snap; i_snap++) {
        ...
        for(i_run = 0; i_run < 2; i_run++) {
            ...
            for(i_group = 0; i_group < n_groups; i_group++) {
                SID_fread_verify(&size, sizeof(int), 1, fp_in);
            ...
            for(i_group = 0; i_group < n_groups; i_group++) {
                ...
                SID_fread_verify(&offset_in, sizeof(unsigned int), 1, fp_in);
                if(offset_size == sizeof(unsigned int)) {
                    ...
                    SID_exit_error("Invalid offset size (%d)", SID_ERROR_LOGIC);
            }
            ...
            if(i_run == 1) {
                ...
                for(i_group = 0, n_sub_total = 0; i_group < n_groups; i_group++) {
                    SID_fread_verify(&n_sub, sizeof(int), 1, fp_in);
