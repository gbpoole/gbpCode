...
int main(int argc, char *argv[]) {
    ...
    for(i_snap = start_snap; i_snap <= stop_snap; i_snap++) {
        ...
        if(i_snap < n_a_list)
            ...
            SID_exit_error("Not enough entries in the expansion factor list (ie %d>=%d).", SID_ERROR_LOGIC, i_snap,
                           n_a_list);
            ...
            if(n_props_all == 0) {
                ...
                for(i_file_props = 0; i_file_props < n_files_props; i_file_props++) {
                    ...
                    if(flag_multifile_properties) {
                        ...
                        if(i_file_props > 0)
                            SID_exit_error("Error looping over non-multifile.", SID_ERROR_LOGIC);
                    ...
                    if((fp_properties_read = fopen(filename_read, "r")) == NULL)
                        SID_exit_error("Could not open file {%s} for reading.", SID_ERROR_IO_OPEN, filename_read);
                    if((fp_properties_write = fopen(filename_write, "w")) == NULL)
                        SID_exit_error("Could not open file {%s} for writing.", SID_ERROR_IO_OPEN, filename_write);
                    // Read header
                    SID_fread_verify(&i_file, sizeof(int), 1, fp_properties_read);
                    SID_fread_verify(&n_files_props, sizeof(int), 1, fp_properties_read);
                    SID_fread_verify(&n_props, sizeof(int), 1, fp_properties_read);
                    SID_fread_verify(&n_props_all, sizeof(int), 1, fp_properties_read);
                    // Write header
                ...
                for(i_file_profs = 0; i_file_profs < n_files_profs; i_file_profs++) {
                    ...
                    if((fp_profiles_write = fopen(filename_write, "w")) == NULL)
                        SID_exit_error("Could not open file {%s} for writing.", SID_ERROR_IO_OPEN, filename_write);
                    // Read header
                    SID_fread_verify(&i_file, sizeof(int), 1, fp_profiles_read);
                    SID_fread_verify(&n_files_profs, sizeof(int), 1, fp_profiles_read);
                    SID_fread_verify(&n_profs, sizeof(int), 1, fp_profiles_read);
                    SID_fread_verify(&n_profs_all, sizeof(int), 1, fp_profiles_read);
                    // Write header
                ...
                for(i_halo = 0, i_halo_props_last = 0, i_halo_profs_last = 0, i_file_props = 0, i_file_profs = 0; i_halo < n_props_all; i_halo++) {
                    ...
                    SID_fread_verify(&n_bins, sizeof(int), 1, fp_profiles_read);
                    SID_fread_verify(&bins, sizeof(halo_profile_bin_info), n_bins, fp_profiles_read);
                    ...
                    */
                    ...
                    // Read profiles
                    halo_properties_info properties;
                    SID_fread_verify(&properties, sizeof(halo_properties_info), 1, fp_properties_read);
                    ...
                    */
                    ...
                    if(properties.position_COM[0] < box_size)
                        properties.position_COM[0] += box_size;
                    if(properties.position_COM[1] < box_size)
                        properties.position_COM[1] += box_size;
                    if(properties.position_COM[2] < box_size)
                        properties.position_COM[2] += box_size;
                    if(properties.position_COM[0] >= box_size)
                        properties.position_COM[0] -= box_size;
                    if(properties.position_COM[1] >= box_size)
                        properties.position_COM[1] -= box_size;
