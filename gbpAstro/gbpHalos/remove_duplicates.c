...
void read_duplicates_file_local(const char *filename_in,
...
void read_duplicates_file_local(const char *filename_in,
    ...
    if((fp_in = fopen(filename_in, "r")) == NULL)
        ...
        (*flag_add_duplicates) = GBP_TRUE;
    }
}

void inject_duplicates_local(plist_info *plist,
                             const char *filename_number,
                             int *       size_group,
                             size_t *    offset_group,
                             int *       size_subgroup,
...
void write_duplicates_local(const char *filename_out_root,
    ...
    SID_Allreduce(SID_IN_PLACE, &n_buffer_alloc, 1, SID_INT, SID_MAX, SID_COMM_WORLD);
    ...
    for(i_rank = 0; i_rank < SID.n_proc; i_rank++) {
        ...
        if(n_buffer > 0) {
            SID_Bcast(buffer_ids, n_buffer, SID_SIZE_T, i_rank, SID_COMM_WORLD);
            SID_Bcast(buffer_group, n_buffer, SID_INT, i_rank, SID_COMM_WORLD);
            SID_Bcast(buffer_subgroup, n_buffer, SID_INT, i_rank, SID_COMM_WORLD);
            SID_Bcast(buffer_r2, n_buffer, SID_REAL, i_rank, SID_COMM_WORLD);
            SID_Bcast(buffer_flag, n_buffer, SID_CHAR, i_rank, SID_COMM_WORLD);
            if(SID.I_am_Master) {
                int i_buffer;
...
void construct_duplicate_list_local(plist_info *plist,
    ...
    for(i_particle = 0, count = 0; i_particle < n_particles_all_in_groups; i_particle += n_buffer) {
        ...
        n_buffer = GBP_MIN(n_buffer_max, (size_t)(n_particles_all_in_groups - i_particle));
        ...
        for(i_buffer = 0; i_buffer < n_buffer; i_buffer++) {
            ...
            buffer_flag_local[i_buffer]  = GBP_FALSE;
        }
        ...
        for(j_particle = 0; j_particle < n_particles_local; j_particle++) {
            ...
            if(index_rank_test_local >= 0 && index_rank_test_local < n_buffer) {
                ...
                buffer_flag_local[index_rank_test_local]  = GBP_TRUE; // This array lets ranks know which particles in the scan are local
            }
        }

        // Doing a global max on the buffer yields the needed buffer on all ranks
        SID_Allreduce(SID_IN_PLACE, buffer_id, n_buffer, SID_SIZE_T, SID_MAX, SID_COMM_WORLD);
    ...
    if(n_duplicates_local > 0) {
        ...
        SID_Allreduce(SID_IN_PLACE, &n_bad, 1, SID_INT, SID_SUM, SID_COMM_WORLD);
        if(n_bad > 0)
    ...
    if(flag_filefound) {
        ...
        if(n_particles_kept != n_particles_rank)
            SID_log_warning("Rank %d did not receive the right number of particles (ie. %d!=%d)",
                            ...
                            n_particles_kept,
                            n_particles_rank);
        SID_Allreduce(&n_particles_rank, &n_particles_all, 1, SID_SIZE_T, SID_SUM, SID_COMM_WORLD);
...
int main(int argc, char *argv[]) {
    ...
    for(i_file = i_file_hi; i_file >= i_file_lo; i_file -= i_file_skip) {
        ...
        for(i_rank = 0; i_rank < SID.n_proc; i_rank++) {
            ...
            SID_Bcast(&group_index_local_offset_comm, 1, SID_INT, i_rank, SID_COMM_WORLD);
            SID_Bcast(&subgroup_index_local_offset_comm, 1, SID_INT, i_rank, SID_COMM_WORLD);
        }
        SID_log("Done.", SID_LOG_CLOSE);
        ...
        int *   subgroup_duplicate_added_to;
        int *   index_duplicate_added_to;
        read_duplicates_file_local("./remove_duplicates_testing.dat",
                                   filename_number,
                                   &plist,
                                   size_groups,
                                   offset_groups,
        ...
        if(n_groups_all > 0) {
            ...
            sort(id_duplicates_local,
                 n_duplicates_local,
                 &id_duplicates_local_index,
                 SID_SIZE_T,
                 SORT_LOCAL,
                 SORT_COMPUTE_INDEX,
                 SORT_COMPUTE_NOT_INPLACE);
            ...
            for(i_rank = 0; i_rank < SID.n_proc; i_rank++) {
                ...
                SID_Bcast(&offset_to_global_bcast, 1, SID_INT, i_rank, SID_COMM_WORLD);
            }
            SID_log("Done.", SID_LOG_CLOSE);
            ...
            char *   flag_keep_local;
            int      n_exchange_max;
            SID_Allreduce(&n_duplicates_local, &n_exchange_max, 1, SID_INT, SID_MAX, SID_COMM_WORLD);
            if(SID.n_proc > 1) {
            ...
            for(i_duplicate = 0; i_duplicate < n_duplicates_local; i_duplicate++)
                flag_keep_local[i_duplicate] = GBP_TRUE;
            for(i_rank = 0; i_rank < SID.n_proc; i_rank++) {
                ...
                if(i_rank == 0) {
                    ...
                    exchange_ring_buffer(&n_duplicates_local, sizeof(int), 1, &n_duplicates_exchange, NULL, i_rank);
                    exchange_ring_buffer(&offset_to_global_local, sizeof(size_t), 1, &offset_to_global_exchange, NULL, i_rank);
                    exchange_ring_buffer(id_duplicates_local, sizeof(size_t), (size_t)n_duplicates_local, id_duplicates_exchange, NULL, i_rank);
                    exchange_ring_buffer(
                        id_duplicates_local_index, sizeof(size_t), (size_t)n_duplicates_local, id_duplicates_exchange_index, NULL, i_rank);
                    exchange_ring_buffer(r2_duplicates_local, sizeof(GBPREAL), (size_t)n_duplicates_local, r2_duplicates_exchange, NULL, i_rank);
                }
                ...
                for(i_duplicate = 0; i_duplicate < n_duplicates_local && n_duplicates_exchange > 0; i_duplicate++) {
                    ...
                    while(id_duplicates_exchange[id_duplicates_exchange_index[j_duplicate]] <
                          id_duplicates_local[id_duplicates_local_index[i_duplicate]]) {
                        j_duplicate++;
                    ...
                    while(id_duplicates_exchange[id_duplicates_exchange_index[j_duplicate]] ==
                        ...
                        if(r2_duplicates_exchange[id_duplicates_exchange_index[j_duplicate]] <
                           r2_duplicates_local[id_duplicates_local_index[i_duplicate]])
                            flag_keep_local[id_duplicates_local_index[i_duplicate]] = GBP_FALSE;
                        // More complicated case of there being an equally good particle (somehow)
                        else if(r2_duplicates_exchange[id_duplicates_exchange_index[j_duplicate]] ==
                                r2_duplicates_local[id_duplicates_local_index[i_duplicate]]) {
            ...
            for(i_particle = 0, j_particle = 0, i_duplicate = 0, n_removed_local = 0;
                i_particle < n_particles_local && i_duplicate < n_duplicates_local;
                i_duplicate++) {
                ...
                if(id_list_local[i_particle] != id_duplicates_local[index_i])
                    SID_log_warning("Could not find a duplicate (id=%lld at index=%lld of %lld) in the ID list.",
                                    SID_ERROR_LOGIC,
                                    id_duplicates_local[index_i],
                                    index_j,
                                    n_particles_local);
                // Keep this duplicate ...
            ...
            SID_Allreduce(&n_removed_local, &n_removed, 1, SID_INT, SID_SUM, SID_COMM_WORLD);
        ...
        j_particle          = 0;
            ...
            // ... else we're keeping the group, but check if it has any subgroups that are too small ...
                ...
                if(n_remove_i > 0) {
                    ...
                    for(k_subgroup = 0, i_particle_head = 0, i_particle_tail = n_particles_subs, n_particles_copied = 0, n_remove_i = 0;
                        k_subgroup < n_subgroups_group[i_group];
                        k_subgroup++) {
        ...
        SID_Allreduce(SID_IN_PLACE, &n_groups_removed, 1, SID_INT, SID_SUM, SID_COMM_WORLD);
        SID_Allreduce(SID_IN_PLACE, &n_subgroups_removed, 1, SID_INT, SID_SUM, SID_COMM_WORLD);
        SID_Allreduce(SID_IN_PLACE, &n_particles_removed, 1, SID_SIZE_T, SID_SUM, SID_COMM_WORLD);
        SID_Allreduce(&n_groups_local, &n_groups_all, 1, SID_INT, SID_SUM, SID_COMM_WORLD);
        SID_Allreduce(&n_subgroups_local, &n_subgroups_all, 1, SID_INT, SID_SUM, SID_COMM_WORLD);
        SID_Allreduce(&n_particles_local, &n_particles_all, 1, SID_SIZE_T, SID_SUM, SID_COMM_WORLD);
        ...
        for(i_rank = 0; i_rank < SID.n_proc; i_rank++) {
            ...
            SID_Bcast(&offset_to_global, 1, SID_SIZE_T, i_rank, SID_COMM_WORLD);
        }
        SID_log("Done.", SID_LOG_CLOSE);
        ...
        SID_Allreduce(SID_IN_PLACE, &n_buffer_alloc, 1, SID_SIZE_T, SID_MAX, SID_COMM_WORLD);
        ...
        char  filename_in_groups[SID_MAX_FILENAME_LENGTH];
        char  filename_in_subgroups[SID_MAX_FILENAME_LENGTH];
        char  filename_in_particles[SID_MAX_FILENAME_LENGTH];
        ...
        if(SID.I_am_Master) {
            ...
            SID_fread_verify(&n_groups_in, sizeof(int), 1, fp_in_groups);
            SID_fread_verify(&n_byte_offsets_groups, sizeof(int), 1, fp_in_groups);
            SID_fread_verify(&n_subgroups_in, sizeof(int), 1, fp_in_subgroups);
            SID_fread_verify(&n_byte_offsets_subgroups, sizeof(int), 1, fp_in_subgroups);
            SID_fread_verify(&n_byte_ids, sizeof(int), 1, fp_in_particles);
            fclose(fp_in_groups);
            fclose(fp_in_subgroups);
            fclose(fp_in_particles);
        }
        SID_Bcast(&n_byte_offsets_groups, 1, SID_INT, SID_MASTER_RANK, SID_COMM_WORLD);
        SID_Bcast(&n_byte_offsets_subgroups, 1, SID_INT, SID_MASTER_RANK, SID_COMM_WORLD);
        SID_Bcast(&n_byte_ids, 1, SID_INT, SID_MASTER_RANK, SID_COMM_WORLD);
        ...
        char   filename_out_particles[SID_MAX_FILENAME_LENGTH];
        ...
        for(i_rank = 0; i_rank < SID.n_proc; i_rank++) {
            ...
            SID_Bcast(&buffer_size, 1, SID_SIZE_T, i_rank, SID_COMM_WORLD);
            SID_Bcast_local(buffer, buffer_size, i_rank, SID_COMM_WORLD);
            if(SID.I_am_Master) {
                size_t n_write;
