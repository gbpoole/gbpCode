...
void read_groups(char *filename_groups_root, int i_file, int mode, plist_info *plist, const char *catalog_name, ...) {
    int     id_byte_size         = 0;
    size_t  n_particles          = 0;
    int     n_particles_group    = 0;
    int     n_groups             = 0;
    int     n_subgroups          = 0;
    int *   n_groups_rank        = NULL;
    int *   n_subgroups_rank     = NULL;
    size_t *n_particles_rank     = NULL;
    int *   n_subgroups_group    = NULL;
    int *   group_id             = NULL;
    int *   group_length         = NULL;
    int *   subgroup_length      = NULL;
    size_t *subgroup_offset      = NULL;
    size_t *group_offset         = NULL;
    size_t *input_id             = NULL;
    int     n_ids_i              = 0;
    size_t  n_ids                = 0;
    int     n_groups_byte_length = 0;
    int     id_in                = 0;
    ...
    int flag_read_groups       = GBP_TRUE;
    int flag_read_ids          = GBP_TRUE;
    ...
    int PHK_min_local_in = INT_MAX;
    int PHK_max_local_in = -1;
    ...
    int   header_size_groups        = 2 * sizeof(int);
    int   header_size_subgroups     = 2 * sizeof(int);
    int   header_size_ids           = 0;
    int   group_offset_byte_size    = 0;
    int   subgroup_offset_byte_size = 0;
    FILE *fp_groups                 = NULL;
    ...
    if((fp_groups = fopen(filename_groups, "r")) != NULL) {
        ...
        fclose(fp_groups);
        SID_Bcast(&n_groups, 1, SID_INT, SID_MASTER_RANK, SID_COMM_WORLD);
        SID_Bcast(&group_offset_byte_size, 1, SID_INT, SID_MASTER_RANK, SID_COMM_WORLD);
    } else
    ...
    if((fp_subgroups = fopen(filename_subgroups, "r")) != NULL) {
        ...
        fclose(fp_subgroups);
        SID_Bcast(&n_subgroups, 1, SID_INT, SID_MASTER_RANK, SID_COMM_WORLD);
        SID_Bcast(&subgroup_offset_byte_size, 1, SID_INT, SID_MASTER_RANK, SID_COMM_WORLD);
    } else
    ...
    if((fp_ids = fopen(filename_ids, "r")) != NULL) {
        ...
        fclose(fp_ids);
        SID_Bcast(&id_byte_size, 1, SID_INT, SID_MASTER_RANK, SID_COMM_WORLD);
        SID_Bcast(&n_ids, 1, SID_SIZE_T, SID_MASTER_RANK, SID_COMM_WORLD);
    ...
    char   filename_in_PHKs[SID_MAX_FILENAME_LENGTH];
    int    n_bits_PHK;
    int *  PHK_groups;
    int *  PHK_subgroups;
    FILE * fp_PHKs                 = NULL;
    ...
    if(flag_PHK_distribute) {
        ...
        if(fp_PHKs != NULL) {
            ...
            if(SID.I_am_Master) {
                SID_fread_verify(&n_groups, sizeof(int), 1, fp_PHKs);
                SID_fread_verify(&n_bits_PHK, sizeof(int), 1, fp_PHKs);
                SID_fread_verify(&n_particles, sizeof(size_t), 1, fp_PHKs);
            ...
            SID_Bcast(&n_groups, 1, SID_INT, SID_MASTER_RANK, SID_COMM_WORLD);
            SID_Bcast(&n_bits_PHK, 1, SID_INT, SID_MASTER_RANK, SID_COMM_WORLD);
            SID_Bcast(&n_particles, 1, SID_SIZE_T, SID_MASTER_RANK, SID_COMM_WORLD);
            SID_log("(%d groups, %lld particles; %d-bit keys)...", SID_LOG_CONTINUE, n_groups, n_particles, n_bits_PHK);

            // Create read buffer
            int   unit_size = 2 * sizeof(int) + sizeof(size_t);
            char *buffer    = (char *)SID_malloc(n_buffer_max * unit_size);
            ...
            int    n_groups_left    = n_groups;
            size_t n_particles_left = n_particles;
            if(PHK_min_local_in < 0 || PHK_max_local_in < 0) {
                int    i_group          = 0;
                int    i_rank           = 0;
                int    i_buffer         = n_buffer_max;
                size_t n_particles_base = 0;
                int    PHK_last         = -1;
                for(; i_rank < SID.n_proc && i_group < n_groups; i_rank++) {
                    ...
                    if(i_rank == SID.My_rank) {
                        ...
                        if(i_buffer >= n_buffer_max) {
                            ...
                            n_seek += n_buffer;
                            i_buffer = 0;
                            SID_fread_verify(buffer, unit_size, n_buffer, fp_PHKs);
                        }
                        int    PHK_i         = ((int *)(&(buffer[i_buffer * unit_size])))[0];
                        int    PHK_index_i   = ((int *)(&(buffer[i_buffer * unit_size + 1 * sizeof(int)])))[0];
                        size_t n_particles_i = ((size_t *)(&(buffer[i_buffer * unit_size + 2 * sizeof(int)])))[0];
                    ...
                    SID_Bcast(&n_seek, 1, SID_INT, i_rank, SID_COMM_WORLD);
                    ...
                    SID_Bcast(buffer, n_buffer * unit_size, SID_CHAR, i_rank, SID_COMM_WORLD);

                    // Bring the particle and group counters up to date
                    SID_Bcast(&n_particles_base, 1, SID_SIZE_T, i_rank, SID_COMM_WORLD);
                    SID_Bcast(&n_groups_left, 1, SID_INT, i_rank, SID_COMM_WORLD);
                    SID_Bcast(&n_particles_left, 1, SID_SIZE_T, i_rank, SID_COMM_WORLD);
                    SID_Bcast(&i_group, 1, SID_INT, i_rank, SID_COMM_WORLD);
                    SID_Bcast(&i_buffer, 1, SID_INT, i_rank, SID_COMM_WORLD);
                    SID_Bcast(&PHK_last, 1, SID_INT, i_rank, SID_COMM_WORLD);
                } // Loop over rank
                ...
                for(int j_rank = 1; i_rank < SID.n_proc; i_rank++, j_rank++) {
                    if(i_rank == SID.My_rank) {
            ...
            // ... else, count the local number of particles and groups for the given range ...
                ...
                if(n_bits_PHK > 0) {
                    for(int i_rank = 0, PHK_last_rank = -1; i_rank < SID.n_proc; i_rank++) {
                        ...
                        SID_Bcast(&PHK_last_rank, 1, SID_INT, i_rank, SID_COMM_WORLD);
                    }
                }
                int    i_group          = 0;
                int    i_buffer         = n_buffer_max;
                size_t n_particles_base = 0;
                int    n_buffer         = 0;
                for(; i_group < n_groups; i_group++, i_buffer++) {
                    // Buffer the read
                    if(i_buffer >= n_buffer_max) {
                        n_buffer = GBP_MIN(n_buffer_max, n_groups - i_group);
                        i_buffer = 0;
                        if(SID.I_am_Master)
                            SID_fread_verify(buffer, unit_size, n_buffer, fp_PHKs);
                        SID_Bcast(buffer, n_buffer * unit_size, SID_CHAR, SID_MASTER_RANK, SID_COMM_WORLD);
                    }
                    int    PHK_i         = ((int *)(&(buffer[i_buffer * unit_size])))[0];
                    int    PHK_index_i   = ((int *)(&(buffer[i_buffer * unit_size + 1 * sizeof(int)])))[0];
                    size_t n_particles_i = ((size_t *)(&(buffer[i_buffer * unit_size + 2 * sizeof(int)])))[0];
                ...
                SID_Allreduce(&n_groups_local, &n_groups_check, 1, SID_INT, SID_SUM, SID_COMM_WORLD);
                SID_Allreduce(&n_particles_local, &n_particles_check, 1, SID_SIZE_T, SID_SUM, SID_COMM_WORLD);
            ...
            if(SID.n_proc > 1) {
                for(int i_rank = 0; i_rank < SID.n_proc; i_rank++) {
                    ...
                    SID_Bcast(&PHK_min_i, 1, SID_INT, i_rank, SID_COMM_WORLD);
                    SID_Bcast(&PHK_max_i, 1, SID_INT, i_rank, SID_COMM_WORLD);
            ...
            merge_sort(read_index_group_temp, (size_t)n_groups_local, &read_index_group_temp_index, SID_INT, SORT_COMPUTE_INDEX, GBP_FALSE);
            ...
            int *PHK_boundary               = (int *)SID_malloc(sizeof(int) * n_groups_boundary); // read ordered
            int *PHK_interior               = (int *)SID_malloc(sizeof(int) * n_groups_interior); // read ordered
            int *local_index_group_boundary = (int *)SID_malloc(sizeof(int) * n_groups_boundary); // read indices
            int *local_index_group_interior = (int *)SID_malloc(sizeof(int) * n_groups_interior); // read indices
            for(int i_group = 0, i_boundary = 0, i_interior = 0; i_group < n_groups_local; i_group++) {
                PHK_t key_temp;
                key_temp = (PHK_t)PHK_groups[i_group];
                if(is_a_member(&key_temp, keys_boundary, n_keys_boundary, SID_PHK_T)) {
            ...
            merge_sort(PHK_boundary, (size_t)n_groups_boundary, &PHK_boundary_index, SID_INT, SORT_COMPUTE_INDEX, GBP_FALSE);
            merge_sort(PHK_interior, (size_t)n_groups_interior, &PHK_interior_index, SID_INT, SORT_COMPUTE_INDEX, GBP_FALSE);
            ...
            j_group             = 0;
            for(int i_group = 0; i_group < n_groups_boundary; i_group++, j_group++) {
            ...
            for(int i_group = 0; i_group < n_groups_interior; i_group++, j_group++) {
    ...
    // ... assign to ranks in rank-ordered blocks (default).
        ...
        if((fp_groups = fopen(filename_groups, "r")) != NULL) {
            ...
            if(SID.I_am_Master) {
                ...
                for(int i_group = 0; i_group < n_groups; i_group++) {
                    SID_fread_verify(&group_length_i, sizeof(int), 1, fp_groups);
            ...
            n_particles_local       = 0;
            int    n_seek           = 0;
            int    last_used_rank   = 0; // default
            int    i_rank           = 0;
            int    i_group          = 0;
            size_t n_particles_left = n_particles;
            fseeko(fp_groups, (off_t)(header_size_groups), SEEK_SET);
            for(; i_rank < SID.n_proc && n_particles_left > 0; i_rank++) {
                if(n_seek > 0)
                ...
                if(i_rank == SID.My_rank) {
                    ...
                    while((i_group < n_groups && n_particles_local < (size_t)((double)(n_particles_left) / (double)(SID.n_proc - i_rank))) ||
                          (group_length_i == 0)) {
                        SID_fread_verify(&group_length_i, sizeof(int), 1, fp_groups);
                        n_groups_local++;
                ...
                SID_Bcast(&n_seek, 1, SID_INT, i_rank, SID_COMM_WORLD);
                SID_Bcast(&i_group, 1, SID_INT, i_rank, SID_COMM_WORLD);
                SID_Bcast(&last_used_rank, 1, SID_INT, i_rank, SID_COMM_WORLD);
                if(i_rank == SID.My_rank)
                    n_seek = 0;
            }
        ...
        read_index_group    = (int *)SID_malloc(sizeof(int) * n_groups_local);
        storage_index_group = (int *)SID_malloc(sizeof(int) * n_groups_local);
        for(int i_rank = 0, i_group = 0; i_rank < SID.n_proc; i_rank++) {
            if(i_rank == SID.My_rank) {
                for(int j_group = 0; j_group < n_groups_local; i_group++, j_group++) {
    ...
    int *read_index_group_store = (int *)SID_malloc(sizeof(int) * n_groups_local);
    for(int i_group = 0; i_group < n_groups_local; i_group++)
    ...
    if(flag_read_groups || flag_read_subgroups) {
        // Read the group file ...
        if(flag_read_groups) {
            SID_log("Reading group file {%s}...%d-byte particle offsets...", SID_LOG_OPEN, filename_groups, group_offset_byte_size);
            if((fp_groups = fopen(filename_groups, "r")) != NULL)
                fseeko(fp_groups, (off_t)header_size_groups, SEEK_SET);
            else
                SID_exit_error("Could not open {%s}!\n", SID_ERROR_LOGIC, filename_groups);
            if(n_groups > 0) {
                ...
                group_offset      = (size_t *)SID_malloc(sizeof(size_t) * n_groups_local);
                n_subgroups_group = (int *)SID_malloc(sizeof(int) * n_groups_local);

                // This array is used to check for indexing inconsistancies
                int *test = (int *)SID_malloc(sizeof(int) * n_groups_local);

                // ... read local group lengths ...
                char *        buffer    = (char *)SID_malloc(GBP_MAX(group_offset_byte_size, subgroup_offset_byte_size) * n_buffer_max);
                unsigned int *buffer_ui = (unsigned int *)buffer;
                int *         buffer_i  = (int *)buffer;
                int64_t *     buffer_ll = (int64_t *)buffer;
                for(int i_test = 0; i_test < n_groups_local; i_test++)
                    test[i_test] = 0;
                int i_group  = 0;
                int j_group  = 0;
                int i_buffer = n_buffer_max;
                ...
                for(; i_group < n_groups; i_group++, i_buffer++) {
                    ...
                    if(i_buffer >= n_buffer_max) {
                        ...
                        n_buffer = GBP_MIN(n_buffer_max, n_groups - i_group);
                        if(SID.I_am_Master)
                            SID_fread_verify(buffer_i, sizeof(int), n_buffer, fp_groups);
                        SID_Bcast(buffer_i, n_buffer, SID_INT, SID_MASTER_RANK, SID_COMM_WORLD);
                    }
                    if(j_group < n_groups_local) {
                        if(read_index_group[j_group] == i_group) {
                            group_length[storage_index_group[j_group]] = buffer_i[i_buffer];
                ...
                if(flag_PHK_distribute) {
                    size_t n_particles_boundary = 0;
                    if(flag_read_MBP_ids_only)
                        n_particles_boundary = n_groups_boundary;
                    else {
                        for(int i_group = 0; i_group < n_groups_boundary; i_group++)
                ...
                for(int i_test = 0; i_test < n_groups_local; i_test++)
                    test[i_test] = 0;
                for(int i_group = 0, j_group = 0, i_buffer = n_buffer_max; i_group < n_groups; i_group++, i_buffer++) {
                    // Buffer the read
                    if(i_buffer >= n_buffer_max) {
                        i_buffer = 0;
                        n_buffer = GBP_MIN(n_buffer_max, n_groups - i_group);
                        if(SID.I_am_Master)
                            SID_fread_verify(buffer, group_offset_byte_size, n_buffer, fp_groups);
                        SID_Bcast(buffer, n_buffer * group_offset_byte_size, SID_CHAR, SID_MASTER_RANK, SID_COMM_WORLD);
                    }
                    if(j_group < n_groups_local) {
                        if(read_index_group[j_group] == i_group) {
                            switch(flag_group_long_offsets) {
                                case GBP_TRUE:
                                    group_offset[storage_index_group[j_group]] = (size_t)(buffer_ll[i_buffer]);
                                    break;
                                case GBP_FALSE:
                                    group_offset[storage_index_group[j_group]] = (size_t)(buffer_ui[i_buffer]);
                                    break;
                            }
                            test[storage_index_group[j_group]]++;
                            j_group++;
                ...
                int *read_index_subgroup_temp = (int *)SID_malloc(sizeof(int) * n_groups_local);
                read_seek_subgroup =
                    (int *)SID_calloc(sizeof(int) * (n_groups_local + 1)); // Must be initialized to zero; one extra needed for end-block syncing
                for(int i_test = 0; i_test < n_groups_local; i_test++)
                    test[i_test] = 0;
                i_group           = 0;
                j_group           = 0;
                ...
                for(; i_group < n_groups; i_group++, i_buffer++) {
                    ...
                    if(i_buffer >= n_buffer_max) {
                        ...
                        n_buffer = GBP_MIN(n_buffer_max, n_groups - i_group);
                        if(SID.I_am_Master)
                            SID_fread_verify(buffer, sizeof(int), n_buffer, fp_groups);
                        SID_Bcast(buffer, n_buffer, SID_INT, SID_MASTER_RANK, SID_COMM_WORLD);
                    }
                    if(j_group < n_groups_local) {
                        if(read_index_group[j_group] == i_group) {
                            n_subgroups_local += buffer_i[i_buffer];
                            n_subgroups_group[storage_index_group[j_group]]        = buffer_i[i_buffer];
                            read_index_subgroup_temp[storage_index_group[j_group]] = i_subgroup;
                            test[storage_index_group[j_group]]++;
                            j_group++;
                        } else
                            read_seek_subgroup[j_group] += buffer_i[i_buffer];
                    } else
                        read_seek_subgroup[j_group] += buffer_i[i_buffer];
                    i_subgroup += buffer_i[i_buffer];
                }
                SID_free(SID_FARG buffer);
                if(j_group != n_groups_local)
                    SID_exit_error(
                ...
                int n_subgroups_check = 0;
                calc_sum(n_subgroups_group, &n_subgroups_local, n_groups_local, SID_INT, CALC_MODE_DEFAULT);
                calc_sum_global(&n_subgroups_local, &n_subgroups_check, 1, SID_INT, CALC_MODE_DEFAULT, SID_COMM_WORLD);
                ...
                if(n_groups_local > 0) {
                    // ... subgroups ...
                    temp_array[0] = 0;
                    for(int i_group = 1; i_group < n_groups_local; i_group++)
                ...
                if(SID.n_proc > 1) {
                    ...
                    for(int i_rank = 0; i_rank < SID.n_proc; i_rank++) {
                        ...
                        SID_Bcast(&n_groups_report, 1, SID_INT, i_rank, SID_COMM_WORLD);
                        SID_Bcast(&n_subgroups_report, 1, SID_INT, i_rank, SID_COMM_WORLD);
                        SID_Bcast(&n_particles_report, 1, SID_SIZE_T, i_rank, SID_COMM_WORLD);
        ...
        if(flag_read_subgroups) {
            SID_log("Reading subgroup file {%s}...%d-byte particle offsets...", SID_LOG_OPEN, filename_subgroups, subgroup_offset_byte_size);
            if((fp_subgroups = fopen(filename_subgroups, "r")) != NULL)
            ...
            if(n_subgroups > 0) {
                ...
                subgroup_offset = (size_t *)SID_malloc(sizeof(size_t) * n_subgroups_local);
                ...
                int i_group = 0;
                int j_group = 0;
                for(; i_group < n_groups && j_group < n_groups_local; i_group++) {
                    if(read_index_group[j_group] == i_group) {
                        ...
                        SID_fread_verify(&(subgroup_length[index_subgroup]), sizeof(int),
                                         n_subgroups_group[index_group], fp_subgroups);
                        for(int i_test = index_subgroup, j_test = 0; j_test < n_subgroups_group[index_group]; i_test++, j_test++)
                            test[i_test]++;
                        j_group++;
                    }
                }
                ...
                for(i_group = 0, j_group = 0; i_group < n_groups && j_group < n_groups_local; i_group++) {
                    if(read_index_group[j_group] == i_group) {
                        ...
                        switch(flag_subgroup_long_offsets) {
                            case GBP_TRUE:
                                for(int i_subgroup = index_subgroup, j_subgroup = 0; j_subgroup < n_subgroups_group[index_group];
                                    i_subgroup++, j_subgroup++) {
                                    SID_fread_verify(&offset_ll, subgroup_offset_byte_size, 1, fp_subgroups);
                            ...
                            case GBP_FALSE:
                                for(int i_subgroup = index_subgroup, j_subgroup = 0; j_subgroup < n_subgroups_group[index_group];
                                    i_subgroup++, j_subgroup++) {
                                    SID_fread_verify(&offset_ui, subgroup_offset_byte_size, 1, fp_subgroups);
                        ...
                        // Sanity Check
                        for(int i_subgroup = 0; i_subgroup < n_subgroups_group[index_group]; i_subgroup++) {
                            subgroup_index_max =
                                (size_t)subgroup_offset[index_subgroup + i_subgroup] + (size_t)subgroup_length[index_subgroup + i_subgroup];
                ...
                // ... sanity check ...
                for(int i_group = 0, i_subgroup = 0; i_group < n_groups_local && i_subgroup < n_subgroups_local;
        ...
        if(flag_read_ids) {
            ...
            if(n_ids > 0) {
                ...
                size_t *buffer;
                int *   buffer_int;
                calc_max_global(group_length, &max_group_length, n_groups_local, SID_INT, CALC_MODE_DEFAULT, SID_COMM_WORLD);
                if(flag_read_MBP_ids_only)
                    max_group_length = 1;
                ...
                size_t *buffer_offsets_tmp   = (size_t *)SID_malloc(sizeof(size_t) * n_buffer_max);
                int *   buffer_ranks         = (int *)SID_malloc(sizeof(int) * n_buffer_max);
                int *   buffer_sizes         = (int *)SID_malloc(sizeof(int) * n_buffer_max);
                size_t *buffer_offsets       = (size_t *)SID_malloc(sizeof(size_t) * n_buffer_max);
                int *   buffer_indices_local = (int *)SID_malloc(sizeof(int) * n_buffer_max);
                int *   buffer_sizes_local   = (int *)SID_malloc(sizeof(int) * n_buffer_max);
                int     i_buffer             = n_buffer_max;
                int     j_group              = 0;
                int     index_group          = 0;
                int     index_particles      = 0;
                if(n_groups_local > 0) {
                    index_group     = storage_index_group[j_group];
                ...
                int    flag_seek       = GBP_TRUE;
                int    n_buffer        = 0;
                for(; i_group < n_groups; i_group += n_buffer) {
                    // Tell the Master rank what needs to be read and where
                    //   it needs to be sent for this buffered chunk
                    n_buffer_local = 0;
                    n_buffer       = GBP_MIN(n_buffer_max, n_groups - i_group);
                    for(int k_group = 0, l_group = i_group; k_group < n_buffer; k_group++, l_group++) {
                        if(j_group < n_groups_local) {
                    ...
                    SID_Reduce(buffer_ranks_tmp, buffer_ranks, n_buffer, SID_INT, SID_MAX, SID_MASTER_RANK, SID_COMM_WORLD);
                    SID_Reduce(buffer_sizes_tmp, buffer_sizes, n_buffer, SID_INT, SID_MAX, SID_MASTER_RANK, SID_COMM_WORLD);
                    SID_Reduce(buffer_offsets_tmp, buffer_offsets, n_buffer, SID_SIZE_T, SID_MAX, SID_MASTER_RANK, SID_COMM_WORLD);
                    if(SID.I_am_Master) {
                    ...
                    if(SID.I_am_Master) {
                        for(int i_buffer = 0, j_buffer = 0; i_buffer < n_buffer; i_buffer++) {
                            fseeko(fp_ids, (off_t)((buffer_offsets[i_buffer] - index_last_read) * id_byte_size), SEEK_CUR);
                            index_last_read = buffer_offsets[i_buffer] + (size_t)buffer_sizes[i_buffer];
                            // Read locally for Master rank
                            if(buffer_ranks[i_buffer] == SID_MASTER_RANK) {
                                switch(flag_long_ids) {
                                    case GBP_TRUE:
                                        SID_fread_verify(&(input_id[buffer_indices_local[j_buffer]]), id_byte_size,
                                                         buffer_sizes_local[j_buffer], fp_ids);
                                        for(size_t i_particle = 0, j_particle = buffer_indices_local[j_buffer];
                                            i_particle < buffer_sizes_local[j_buffer];
                                            i_particle++, j_particle++)
                                            test[j_particle]++;
                                        break;
                                    default:
                                        SID_fread_verify(buffer_int, id_byte_size, buffer_sizes_local[j_buffer], fp_ids);
                                        for(size_t i_particle = 0, j_particle = buffer_indices_local[j_buffer];
                                            i_particle < buffer_sizes_local[j_buffer];
                                            i_particle++, j_particle++) {
                            ...
                            // Read and send to another rank
                                ...
                                switch(flag_long_ids) {
                                    case GBP_TRUE:
                                        SID_fread_verify(buffer, id_byte_size, buffer_sizes[i_buffer], fp_ids);
                                        break;
                                    default:
                                        SID_fread_verify(buffer_int, id_byte_size, buffer_sizes[i_buffer], fp_ids);
                                        for(size_t i_particle = 0; i_particle < buffer_sizes[i_buffer]; i_particle++)
                ...
                for(int i_group = 0, i_subgroup = 0; i_group < n_groups_local && i_subgroup < n_subgroups_local; i_group++) {
