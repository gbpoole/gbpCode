...
void match_halos(plist_info *plist_1_in,
    ...
    char catalog_1[64];
    char catalog_2[64];
    if(catalog_1_in == NULL)
        sprintf(catalog_1, "%03d", i_file_1);
    else
        strcpy(catalog_1, catalog_1_in);
    if(catalog_2_in == NULL)
        sprintf(catalog_2, "%03d", i_file_2);
    else
        strcpy(catalog_2, catalog_2_in);
    ...
    if(check_mode_for_flag(mode, MATCH_SUBSTRUCTURE)) {
        ...
        flag_match_substructure = GBP_FALSE;
    ...
    if(flag_match_subgroups) {
        SID_log("Matching subgroups from catalog {%s} to catalog {%s}...", SID_LOG_OPEN | SID_LOG_TIMER, catalog_1, catalog_2);
    ...
    if(n_groups_1_all > 0 && n_groups_2_all > 0) {
        ...
        if(mark_list_1 == NULL)
            ...
            merge_sort((void *)mark_list_1, (size_t)n_mark_1_local, &mark_list_index_1, SID_INT, SORT_COMPUTE_INDEX, GBP_FALSE);
        ...
        if(mark_list_2_local == NULL) {
            ...
            merge_sort((void *)mark_list_2_local, (size_t)n_mark_2_local, &mark_list_index_2_local, SID_INT, SORT_COMPUTE_INDEX, GBP_FALSE);
        ...
        for(i_group = 0; i_group < n_groups_1; i_group++) {
            ...
            if(flag_read_marked) {
                if(mark_list_index_1 != NULL) {
                    if(mark_list_1[mark_list_index_1[i_mark]] == i_group) {
                        flag_match_this = GBP_TRUE;
                    } else {
                        flag_match_this = GBP_FALSE;
                    }
                } else {
        ...
        for(i_rank = 0, n_match = 0; i_rank < SID.n_proc; i_rank++) {
            ...
            if(i_rank == 0) {
                ...
                index_1       = NULL;
                index_2_local = NULL;
                merge_sort(id_1, (size_t)(n_particles_1), &index_1, SID_SIZE_T, SORT_COMPUTE_INDEX, GBP_FALSE);
                merge_sort(id_2_local, (size_t)(n_particles_2_local), &index_2_local, SID_SIZE_T, SORT_COMPUTE_INDEX, GBP_FALSE);
            ...
            // We just need to sort the boundary particles once
                ...
                merge_sort(id_1, (size_t)(n_particles_exchange_1), &index_1, SID_SIZE_T, SORT_COMPUTE_INDEX, GBP_FALSE);
                merge_sort(id_2_local, (size_t)(n_particles_exchange_2), &index_2_local, SID_SIZE_T, SORT_COMPUTE_INDEX, GBP_FALSE);
                SID_log("Done.", SID_LOG_CLOSE);
            }
            ...
            // Create buffer arrays and perform exchanges if matching against another rank
                ...
                exchange_ring_buffer(n_particles_group_2_local, sizeof(int), (size_t)n_groups_2_local, n_particles_group_2, NULL, i_rank);
                SID_log("group indices...", SID_LOG_COMMENT);
                exchange_ring_buffer(group_index_2_local, sizeof(int), (size_t)n_particles_2_local, group_index_2, NULL, i_rank);
                SID_log("file indices...", SID_LOG_COMMENT);
                exchange_ring_buffer(file_index_2_local, sizeof(int), (size_t)n_groups_2_local, file_index_2, NULL, i_rank);
                SID_log("ids...", SID_LOG_COMMENT);
                exchange_ring_buffer(id_2_local, sizeof(size_t), (size_t)n_particles_2_local, id_2, NULL, i_rank);
                SID_log("sort indices...", SID_LOG_COMMENT);
                exchange_ring_buffer(index_2_local, sizeof(size_t), (size_t)n_particles_2_local, index_2, NULL, i_rank);
                SID_log("Done.", SID_LOG_CLOSE);
            }
            ...
            for(i_particle = 0, j_particle = 0; i_particle < n_particles_1 && j_particle < n_particles_2; i_particle++, flag_use_bisect = GBP_FALSE) {
                ...
                if(hist_size > 0) {
                    ...
                    if(j_particle < (n_particles_2 - 1)) {
                        while(id_1_i > id_2[index_2[j_particle]] && j_particle < (n_particles_2 - 2))
                            j_particle++;
                    ...
                    if(id_1_i == id_2[idx_2] && j_group >= 0) {
                        ...
                        switch(flag_match_substructure) {
                            case GBP_TRUE:
                                for(i_hist = 0, flag = GBP_TRUE; i_hist < n_hist_array[i_group] && flag; i_hist++) {
                        ...
                        if(flag) {
                            ...
                            switch(flag_match_substructure) {
                                case GBP_TRUE:
                                    if(n_particles_group_1[i_group] < n_particles_group_2[j_group]) {
            ...
            if(hist_size_array[i_group] > 0) { // not true if this halo has been flaged as not to be matched
                ...
                if(n_hist_array[i_group] > 0) {
                    ...
                    switch(flag_match_substructure) {
                        ...
                        case GBP_TRUE:
                            for(i_hist = 1; i_hist < n_hist_array[i_group]; i_hist++) {
                                if(n_particles_group_2[hist_list[i_hist]] < n_particles_group_2[hist_list[j_hist]])
                                    j_hist = i_hist;
                            }
                            break;
                        ...
                        default:
                            ...
                            if(flag_moment_preselection) {
                                ...
                                for(i_hist = 0; i_hist < n_hist_array[i_group]; i_hist++) {
                                    if(hist_count[i_hist] >= N_P_MATCH_PRESELECT_MIN) {
                                        float f_m1_new = match_score_f_goodness(hist_score[i_hist], n_particles_group_1[i_group]);
                                        float f_0_new  = ((float)hist_count[i_hist] / (float)n_particles_group_1[i_group]);
                                        float f_m1_old = match_score_f_goodness(hist_score[j_hist], n_particles_group_1[i_group]);
                                        float f_0_old  = ((float)hist_count[j_hist] / (float)n_particles_group_1[i_group]);
                                        if((f_m1_new / f_0_new) > (f_m1_old / f_0_old))
                                            j_hist = i_hist;
                                    }
                                }
                            }
                    ...
                    if(match[i_mark] < (-1) || match[i_mark] >= n_groups_2_all) {
                        SID_log_warning(
                            "Invalid match_id (%d) for i_mark/i_group=%d/%d.  There are %d objects in the target catalog. n_hist/j_hist=%d/%d",
                            SID_ERROR_LOGIC,
                            match[i_mark],
                            i_mark,
                            i_group,
