...
int main(int argc, char *argv[]) {
    ...
    for(i_file = i_file_lo; i_file <= i_file_hi; i_file += i_file_skip) {
        ...
        for(i_process = 0; i_process < 2; i_process++) {
            ...
            j_file = 0;
            sprintf(filename_input_properties_temp,
                    "%s/%s.catalog_%sgroups_properties.%d",
                    filename_input_properties_dir,
                    filename_base,
                    group_text_prefix,
                    j_file);
            ...
            for(j_file = 0; j_file < n_files; j_file++) {
                ...
                sprintf(filename_input_properties_temp,
                        ...
                        group_text_prefix,
                        j_file);
                sprintf(filename_input_profiles_temp,
                        "%s/%s.catalog_%sgroups_profiles.%d",
                        filename_input_properties_dir,
                        filename_base,
                        group_text_prefix,
                        j_file);
