...
int fopen_nth_catalog_file(fp_catalog_info *fp_in, int n) {
    ...
    while(i_file <= n && !r_val) {
        ...
        if(fp_in->flag_read_properties) {
            if(((fp_in->fp_properties) = fopen(filename_properties, "r")) == NULL)
                ...
                if(i_file != fp_in->i_file)
                    SID_trap_error(
                        "Invalid file number (ie. %d!=%d) in catalog {%s}.", ERROR_LOGIC, i_file, fp_in->i_file, fp_in->filename_properties_root);
            }
        }
        ...
        if(fp_in->flag_read_profiles) {
            if(((fp_in->fp_profiles) = fopen(filename_profiles, "r")) == NULL)
                ...
                if(i_file != fp_in->i_file)
                    SID_trap_error(
                        "Invalid file number (ie. %d!=%d) in catalog {%s}.", ERROR_LOGIC, i_file, fp_in->i_file, fp_in->filename_profiles_root);
            }
        }
    ...
    if(r_val == TRUE)
        SID_trap_error(
            "Problem encountered opening file {%s/%s;file=%d}", ERROR_LOGIC, fp_in->filename_properties_base, fp_in->filename_profiles_base, n);
    ...
    if(SID.I_am_Master) {
        ...
        if(fp_out->fp_properties == NULL) {
            ...
            if(fp_out->fp_properties == NULL) {
                r_val = TRUE;
                SID_trap_error(
                    "Could not open catalog {%s} snapshot #%03d {%s}.", ERROR_IO_OPEN, filename_catalog_root, snapshot_number, filename_properties);
            }
