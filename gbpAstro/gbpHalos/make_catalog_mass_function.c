...
int main(int argc, char *argv[]) {
    ...
    SID_log("Generating mass functions for catalogs {%s;%s}, snaps %d->%d...",
    ...
    if(SID.I_am_Master) {
        ...
        for(int snap_number = snap_number_start; snap_number <= snap_number_stop; snap_number++) {
            ...
            fopen_catalog(
                filename_cat_root, snap_number, READ_CATALOG_GROUPS | READ_CATALOG_PROPERTIES | READ_CATALOG_PROPERTIES, &fp_catalog_groups);
            fopen_catalog(
                filename_cat_root, snap_number, READ_CATALOG_SUBGROUPS | READ_CATALOG_PROPERTIES | READ_CATALOG_PROPERTIES, &fp_catalog_subgroups);

            // Open SO files if they're available
            float *           group_SO_data = NULL;
            fp_multifile_info fp_SO;
            int               flag_use_SO =
                fopen_multifile("%s/catalogs/%s_%03d.catalog_groups_SO", sizeof(float), &fp_SO, filename_SSimPL, filename_halo_type, snap_number);
            if(flag_use_SO) {
            ...
            for(int i_group = 0, i_subgroup = 0; i_group < n_groups; i_group++) {
                ...
                for(int j_subgroup = 0; j_subgroup < n_subgroups_group; i_subgroup++, j_subgroup++) {
                    ...
                    subgroups[i_subgroup] =
                        m_dark * (double)properties_subgroup->n_particles * (1. - pow((double)properties_subgroup->n_particles, -0.6));
            ...
            for(int i_type = 0; i_type < (i_rank_max + 2); i_type++) {
                ...
                if(n_data > 0)
                    while(data[data_idx[i_data]] < lM_min && i_data < n_data)
                        i_data++;
            ...
            for(int i = 0; i < n_bins; i++) {
                double dn_dlogM_theory_1 =
                    mass_function(take_alog10(bin_median_groups[i]) * M_sol_inv_h, redshift, &cosmo, MF_ST) * pow(Mpc_inv_h, 3.0);
                double n_theory_1 = mass_function_cumulative(take_alog10(bin[i]) * M_sol_inv_h, redshift, &cosmo, MF_ST) * pow(Mpc_inv_h, 3.0);
                double dn_dlogM_theory_2 =
                    mass_function(take_alog10(bin_median_groups[i]) * M_sol_inv_h, redshift, &cosmo, MF_WATSON) * pow(Mpc_inv_h, 3.0);
                double n_theory_2 = mass_function_cumulative(take_alog10(bin[i]) * M_sol_inv_h, redshift, &cosmo, MF_WATSON) * pow(Mpc_inv_h, 3.0);
                if(hist_list[0][i] > 0) {
                    int cumulative_hist_groups = 0;
                    for(int j_bin = i; j_bin < n_bins; j_bin++)
                        cumulative_hist_groups += hist_list[0][j_bin];
                    fprintf(fp_out,
                            "%11.4le %11.4le %11.4le %6d %11.4le %11.4le %10.4le %10.4le %6d %10.4le %10.4le %10.4le %10.4le\n",
                            bin[i],
                            bin_median_groups[i],
                            bin[i + 1],
                            hist_list[0][i],
            ...
            for(int i_type = 1; i_type < (i_rank_max + 2); i_type++) {
                ...
                if(count > 0) {
                    ...
                    SID_log("Writing results to {%s}...", SID_LOG_OPEN | SID_LOG_TIMER, filename_out);
                    fprintf(fp_out,
                            "# Substructure mass function for snapshot %d of catalog {%s;%s}\n",
                            snap_number,
                            filename_SSimPL,
                            filename_halo_type);
                    fprintf(fp_out, "# Column (01): M_lo     [source units]\n");
                    ...
                    for(int i = 0; i < n_bins; i++) {
                        if(hist_list[i_type][i] > 0) {
                            ...
                            fprintf(fp_out,
                                    "%11.4le %11.4le %11.4le %6d %10.4le %10.4le %6d %10.4le %10.4le\n",
                                    bin[i],
