...
int check_if_substructure_hierarchy_defined(const char *filename_SSimPL_root, const char *filename_halos_version, int i_snap) {
    ...
    if(SID.I_am_Master) {
        ...
        int   n_subgroups_in;
        SID_fread_verify(&n_subgroups_in, sizeof(int), 1, fp_test);
        int offset_size;
        SID_fread_verify(&offset_size, sizeof(int), 1, fp_test);
        fseeko(fp_test, n_subgroups_in * (sizeof(int) + offset_size), SEEK_CUR);
        int test;
        fread(&offset_size, sizeof(int), 1, fp_test); // make sure not to verify here!
        flag_read_sub_pointers = !feof(fp_test);
        fclose(fp_test);
    }
    SID_Bcast(&flag_read_sub_pointers, 1, SID_INT, SID_MASTER_RANK, SID_COMM_WORLD);
