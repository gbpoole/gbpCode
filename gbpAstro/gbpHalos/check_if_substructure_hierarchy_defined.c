...
int check_if_substructure_hierarchy_defined(const char *filename_SSimPL_root, const char *filename_halos_version, int i_snap) {
    ...
    if(SID.I_am_Master) {
        ...
        int   n_subgroups_in;
        fread_verify(&n_subgroups_in, sizeof(int), 1, fp_test);
        int offset_size;
        fread_verify(&offset_size, sizeof(int), 1, fp_test);
    ...
    SID_Bcast(&flag_read_sub_pointers, 1, SID_INT, SID_MASTER_RANK, SID.COMM_WORLD);
