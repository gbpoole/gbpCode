...
int compute_group_analysis(halo_properties_info *properties,
    ...
    if(n_particles == 0)
        ...
        if(flag_manual_centre) {
            ...
            if(properties->position_MBP[0] < box_size)
                properties->position_MBP[0] += box_size;
            if(properties->position_MBP[1] < box_size)
                properties->position_MBP[1] += box_size;
            if(properties->position_MBP[2] < box_size)
                properties->position_MBP[2] += box_size;
            if(properties->position_MBP[0] >= box_size)
                properties->position_MBP[0] -= box_size;
            if(properties->position_MBP[1] >= box_size)
                properties->position_MBP[1] -= box_size;
        ...
        for(j_particle = 0; j_particle < n_particles; j_particle++) {
            ...
            x[j_particle] =
                expansion_factor * d_periodic(p_i_fctn(params, 0, j_particle) - x_cen, box_size); //((double)x_array[k_particle])-x_cen,box_size);
            y[j_particle] =
                expansion_factor * d_periodic(p_i_fctn(params, 1, j_particle) - y_cen, box_size); //((double)y_array[k_particle])-y_cen,box_size);
            z[j_particle] =
                expansion_factor * d_periodic(p_i_fctn(params, 2, j_particle) - z_cen, box_size); //((double)z_array[k_particle])-z_cen,box_size);
        ...
        if(flag_interpolated == TRUE) {
            ...
            if(flag_interpolated == FLAG_INTERP_MIN_BIN)
                i_profile = 0;
            else if(flag_interpolated == FLAG_INTERP_MAX_BIN)
                i_profile = profile->n_bins - 1;
            else
                SID_trap_error("Unrecognized value for flag_interpolated {%d}.", flag_interpolated);
        ...
        for(i_bin = 0, i_particle = 0; i_bin < profile->n_bins; i_bin++, i_particle += n_in_bin) {
            ...
            for(j_particle = 0; j_particle < n_in_bin; j_particle++) {
                ...
                if(R[k_particle] > 0.) {
                    ...
                    v_rad = (x[k_particle] * (vx[k_particle] - v_x_mean) + y[k_particle] * (vy[k_particle] - v_y_mean) +
                             z[k_particle] * (vz[k_particle] - v_z_mean)) /
                            R[k_particle];
        ...
        if(profile->n_bins > 1) {
            ...
            if(k_profile > 0) {
                ...
                if(i_profile < j_profile && j_profile < k_profile) {
                    ...
                    interpolate_maximum(V_R_interpolate, r_c[i_profile], r_c[j_profile], r_c[k_profile], 0.05, &R_max, &V_max);
