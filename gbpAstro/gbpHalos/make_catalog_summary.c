...
int main(int argc, char *argv[]) {
    ...
    for(snap_number = snap_number_start; snap_number <= snap_number_stop; snap_number++) {
        ...
        fopen_catalog(filename_in_root, snap_number, READ_CATALOG_GROUPS | READ_CATALOG_PROPERTIES, &fp_catalog_groups);
        fopen_catalog(filename_in_root, snap_number, READ_CATALOG_SUBGROUPS | READ_CATALOG_PROPERTIES, &fp_catalog_subgroups);
        ...
        for(; i_group < fp_catalog_groups.n_halos_total; i_group++) {
            ...
            int n_subgroups;
            SID_fread_verify(&n_subgroups, sizeof(int), 1, fp_groups);
