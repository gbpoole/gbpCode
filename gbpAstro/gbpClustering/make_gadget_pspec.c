...
void read_gadget_binary_local(char *filename_root_in, int snapshot_number, int i_coord, slab_info *slab, cosmo_info *cosmo, plist_info *plist);
void read_gadget_binary_local(char *filename_root_in, int snapshot_number, int i_coord, slab_info *slab, cosmo_info *cosmo, plist_info *plist) {
    ...
    */
    ...
    if(n_total > 0) {
        ...
        for(i_run = 0; i_run < 4; i_run++) {
            ...
            read_gadget_binary_local(filename_in_root, snapshot_number, i_run, &(pspec[0].FFT.slab), cosmo, &plist);
