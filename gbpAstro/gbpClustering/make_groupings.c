...
int main(int argc, char *argv[]) {
    ...
    fopen_catalog(filename_cat_root, i_snap, READ_CATALOG_GROUPS | READ_CATALOG_PROPERTIES, &fp_properties_groups);
    fopen_catalog(filename_cat_root, i_snap, READ_CATALOG_SUBGROUPS | READ_CATALOG_PROPERTIES, &fp_properties_subgroups);
    ...
    for(i_grouping = 0, V_grouping = V_max_lo; i_grouping <= n_groupings; i_grouping++, V_grouping += delta_V_max) {
        ...
        if(i_grouping < n_groupings) {
            i_halo = 0;
            while(V_halos[V_halos_index[i_halo]] < V_grouping && i_halo < (n_halos - 1))
                i_halo++;
            if(i_halo > 0) {
        ...
        for(j_halo = 0; j_halo < n_halos_this_grouping && i_halo < n_halos; i_halo++, j_halo++)
            fprintf(fp,
                    "%d %d %d %d %d %le %le %le %le %le %le %le %le %le %le %le %le %le %le %le %le %le\n",
                    i_snap,
