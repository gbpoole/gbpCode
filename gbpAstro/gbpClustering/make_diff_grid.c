...
int main(int argc, char *argv[]) {
    ...
    double Lx1, Ly1, Lz1;
    fp1 = fopen(argv[1], "r");
    SID_fread_verify(&nx1, sizeof(int), 1, fp1);
    SID_fread_verify(&ny1, sizeof(int), 1, fp1);
    SID_fread_verify(&nz1, sizeof(int), 1, fp1);
    SID_fread_verify(&Lx1, sizeof(double), 1, fp1);
    SID_fread_verify(&Ly1, sizeof(double), 1, fp1);
    SID_fread_verify(&Lz1, sizeof(double), 1, fp1);
    SID_fread_verify(&ng1, sizeof(int), 1, fp1);
    SID_fread_verify(&as1, sizeof(int), 1, fp1);

    FILE * fp2;
    int    nx2, ny2, nz2, ng2, as2;
    double Lx2, Ly2, Lz2;
    fp2 = fopen(argv[2], "r");
    SID_fread_verify(&nx2, sizeof(int), 1, fp2);
    SID_fread_verify(&ny2, sizeof(int), 1, fp2);
    SID_fread_verify(&nz2, sizeof(int), 1, fp2);
    SID_fread_verify(&Lx2, sizeof(double), 1, fp2);
    SID_fread_verify(&Ly2, sizeof(double), 1, fp2);
    SID_fread_verify(&Lz2, sizeof(double), 1, fp2);
    SID_fread_verify(&ng2, sizeof(int), 1, fp2);
    SID_fread_verify(&as2, sizeof(int), 1, fp2);
    ...
    for(i_grid = 0; i_grid < ng1; i_grid++) {
        SID_fread_verify(grid_identifier_1, sizeof(char), GRID_IDENTIFIER_SIZE, fp1);
        SID_fread_verify(grid_identifier_2, sizeof(char), GRID_IDENTIFIER_SIZE, fp2);
        ...
        for(i_x = 0; i_x < nx1; i_x++) {
            for(i_y = 0; i_y < ny1; i_y++) {
                for(i_z = 0; i_z < nz1; i_z++) {
                    SID_fread_verify(&d1, sizeof(GBPREAL), 1, fp1);
                    SID_fread_verify(&d2, sizeof(GBPREAL), 1, fp2);
                    if(d2 != d1)
