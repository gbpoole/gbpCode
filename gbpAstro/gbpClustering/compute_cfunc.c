...
int add_pair_CFUNC_local(double x_i, double y_i, double z_i, double x_j, double y_j, double z_j, int zone_i, int zone_j, int mode, cfunc_info *cfunc);
int add_pair_CFUNC_local(double      x_i,
                         double      y_i,
                         double      z_i,
                         double      x_j,
                         double      y_j,
...
void calc_pairs_local(const char *species_name1, const char *species_name2, int i_rank, int i_run, int mode, plist_info *plist, cfunc_info *cfunc);
void calc_pairs_local(const char *species_name1, const char *species_name2, int i_rank, int i_run, int mode, plist_info *plist, cfunc_info *cfunc) {
    ...
    for(i_data1 = 0; i_data1 < n_data1_local; i_data1++) {
        ...
        if(PHK_last_local != PHK_data1_local[index_i]) {
            ...
            compute_PHK_volume_keys(cfunc->n_bits_PHK, (PHK_t)PHK_last_local, 0, cfunc->PHK_width, &n_PHK_volume, &PHK_volume);
            ...
            for(i_key_rank = 0, n_key_rank_use = 0; i_key_rank < (size_t)n_PHK_volume; i_key_rank++) {
                ...
                if(PHK_data2_rank[index_j] == PHK_volume[i_key_rank])
                    n_key_rank_use++;
            }
            ...
            if(flag_self_match) {
                while(PHK_data2_rank[index_j] == PHK_i) {
                    if((i_rank == 0 && index_i < index_j) || SID.My_rank < ((SID.My_rank + i_rank) % SID.n_proc)) { // Avoids double counting
                        add_pair_CFUNC_local(x_data1_local[index_i],
                                             y_data1_local[index_i],
                                             z_data1_local[index_i],
                                             x_data2_rank[index_j],
                                             y_data2_rank[index_j],
                                             z_data2_rank[index_j],
                                             zone_data1_local[index_i],
                                             zone_data2_rank[index_j],
                                             flag_pair_type,
                                             cfunc);
                    }
                    j_data2++;
                    if(j_data2 >= n_data2_rank)
                        break;
                    index_j = PHK_idx_data2_rank[j_data2];
                }
            } else {
...
void compute_cfunc(plist_info *plist, const char *species_name, const char *random_name, cfunc_info *cfunc, int i_run) {
    int        i_bin, j_bin;
    int        i_jack;
    ...
    for(i_bin = 0; i_bin < n_1D; i_bin++) {
        CFUNC_1D[i_bin]  = calc_CFUNC_local((double)DD_1D[0][i_bin], (double)DR_1D[0][i_bin], (double)RR_1D[0][i_bin], cfunc);
        CFUNC_l1D[i_bin] = calc_CFUNC_local((double)DD_l1D[0][i_bin], (double)DR_l1D[0][i_bin], (double)RR_l1D[0][i_bin], cfunc);
    }
    for(i_bin = 0; i_bin < n_2D * n_2D; i_bin++)
        CFUNC_2D[i_bin] = calc_CFUNC_local((double)DD_2D[0][i_bin], (double)DR_2D[0][i_bin], (double)RR_2D[0][i_bin], cfunc);
    SID_log("Done.", SID_LOG_CLOSE);
    ...
    for(i_bin = 0; i_bin < n_1D; i_bin++) {
        for(i_jack = 1, bar_i = 0.; i_jack <= n_jack_total; i_jack++) {
            bar_i += calc_CFUNC_local((double)DD_l1D[i_jack][i_bin], (double)DR_l1D[i_jack][i_bin], (double)RR_l1D[i_jack][i_bin], cfunc);
        }
        bar_i /= (double)n_jack_total;
        for(j_bin = 0; j_bin < n_1D; j_bin++) {
            for(i_jack = 1, bar_j = 0., bar_ij = 0.; i_jack <= n_jack_total; i_jack++) {
                bar_j += calc_CFUNC_local((double)DD_l1D[i_jack][j_bin], (double)DR_l1D[i_jack][j_bin], (double)RR_l1D[i_jack][j_bin], cfunc);
                bar_ij += calc_CFUNC_local((double)DD_l1D[i_jack][i_bin], (double)DR_l1D[i_jack][i_bin], (double)RR_l1D[i_jack][i_bin], cfunc) *
                          calc_CFUNC_local((double)DD_l1D[i_jack][j_bin], (double)DR_l1D[i_jack][j_bin], (double)RR_l1D[i_jack][j_bin], cfunc);
            }
    ...
    for(i_bin = 0; i_bin < n_1D; i_bin++) {
        for(i_jack = 1, bar_i = 0.; i_jack <= n_jack_total; i_jack++) {
            bar_i += calc_CFUNC_local((double)DD_1D[i_jack][i_bin], (double)DR_1D[i_jack][i_bin], (double)RR_1D[i_jack][i_bin], cfunc);
        }
        bar_i /= (double)n_jack_total;
        for(j_bin = 0; j_bin < n_1D; j_bin++) {
            for(i_jack = 1, bar_j = 0., bar_ij = 0.; i_jack <= n_jack_total; i_jack++) {
                bar_j += calc_CFUNC_local((double)DD_1D[i_jack][j_bin], (double)DR_1D[i_jack][j_bin], (double)RR_1D[i_jack][j_bin], cfunc);
                bar_ij += calc_CFUNC_local((double)DD_1D[i_jack][i_bin], (double)DR_1D[i_jack][i_bin], (double)RR_1D[i_jack][i_bin], cfunc) *
                          calc_CFUNC_local((double)DD_1D[i_jack][j_bin], (double)DR_1D[i_jack][j_bin], (double)RR_1D[i_jack][j_bin], cfunc);
            }
            bar_j /= (double)n_jack_total;
            bar_ij /= (double)n_jack_total;
    ...
    for(i_bin = 0; i_bin < n_2D_total; i_bin++) {
        for(i_jack = 1, bar_i = 0.; i_jack <= n_jack_total; i_jack++) {
            bar_i += calc_CFUNC_local((double)DD_2D[i_jack][i_bin], (double)DR_2D[i_jack][i_bin], (double)RR_2D[i_jack][i_bin], cfunc);
        }
        bar_i /= (double)n_jack_total;
        for(j_bin = 0; j_bin < n_2D_total; j_bin++) {
            for(i_jack = 1, bar_j = 0., bar_ij = 0.; i_jack <= n_jack_total; i_jack++) {
                bar_j += calc_CFUNC_local((double)DD_2D[i_jack][j_bin], (double)DR_2D[i_jack][j_bin], (double)RR_2D[i_jack][j_bin], cfunc);
                bar_ij += calc_CFUNC_local((double)DD_2D[i_jack][i_bin], (double)DR_2D[i_jack][i_bin], (double)RR_2D[i_jack][i_bin], cfunc) *
                          calc_CFUNC_local((double)DD_2D[i_jack][j_bin], (double)DR_2D[i_jack][j_bin], (double)RR_2D[i_jack][j_bin], cfunc);
            }
            bar_j /= (double)n_jack_total;
            bar_ij /= (double)n_jack_total;
    ...
    for(i_bin = 0; i_bin < n_1D; i_bin++) {
        for(i_jack = 1, mean = 0.; i_jack <= n_jack_total; i_jack++)
            mean += calc_CFUNC_local((double)DD_l1D[i_jack][i_bin], (double)DR_l1D[i_jack][i_bin], (double)RR_l1D[i_jack][i_bin], cfunc);
        mean /= (double)n_jack_total;
        for(i_jack = 1, std_dev = 0.; i_jack <= n_jack_total; i_jack++)
            std_dev +=
                pow(mean - calc_CFUNC_local((double)DD_l1D[i_jack][i_bin], (double)DR_l1D[i_jack][i_bin], (double)RR_l1D[i_jack][i_bin], cfunc), 2.);
        std_dev           = sqrt(std_dev / (double)n_jack_total);
        dCFUNC_l1D[i_bin] = std_dev * sqrt((double)(n_jack_total - 1));
        for(i_jack = 1, mean = 0.; i_jack <= n_jack_total; i_jack++)
            mean += calc_CFUNC_local((double)DD_1D[i_jack][i_bin], (double)DR_1D[i_jack][i_bin], (double)RR_1D[i_jack][i_bin], cfunc);
        mean /= (double)n_jack_total;
        for(i_jack = 1, std_dev = 0.; i_jack <= n_jack_total; i_jack++)
            std_dev +=
                pow(mean - calc_CFUNC_local((double)DD_1D[i_jack][i_bin], (double)DR_1D[i_jack][i_bin], (double)RR_1D[i_jack][i_bin], cfunc), 2.);
        std_dev          = sqrt(std_dev / (double)n_jack_total);
    ...
    for(i_bin = 0; i_bin < n_2D * n_2D; i_bin++) {
        for(i_jack = 1, mean = 0.; i_jack <= n_jack_total; i_jack++)
            mean += calc_CFUNC_local((double)DD_2D[i_jack][i_bin], (double)DR_2D[i_jack][i_bin], (double)RR_2D[i_jack][i_bin], cfunc);
        mean /= (double)n_jack_total;
        for(i_jack = 1, std_dev = 0.; i_jack <= n_jack_total; i_jack++)
            std_dev +=
                pow(mean - calc_CFUNC_local((double)DD_2D[i_jack][i_bin], (double)DR_2D[i_jack][i_bin], (double)RR_2D[i_jack][i_bin], cfunc), 2.);
        std_dev          = sqrt(std_dev / (double)n_jack_total);
