...
void read_atable(const char *filename_in,
                 plist_info *plist,
                 int         x_column,
                 int         y_column,
                 int         z_column,
    ...
    if(check_mode_for_flag(mode, READ_GROUPING_SLAB)) {
        ...
        for(i_halo = 0, n_halos_local = 0; i_halo < n_halos; i_halo++) {
            ...
            SID_Allreduce(SID_IN_PLACE, &flag_test, 1, SID_INT, SID_SUM, SID_COMM_WORLD);
            if(flag_test != 1) {
        ...
        SID_log("Using %d bit-per-dimension keys (%d keys; size=%le [Mpc/h]).",
                SID_LOG_COMMENT,
                n_bits_PHK,
                (int)PHK_N_KEYS_3D(n_bits_PHK),
                box_size / pow(2., (double)(n_bits_PHK)));
        ...
        if(SID.n_proc > 1) {
            ...
            for(i_halo = 0, n_halos_allocate = 0; i_halo < n_halos; i_halo++) {
                if(i_halo % SID.n_proc == SID.My_rank)
                    n_halos_allocate++; // Just for setting PHKs
            }
        ...
        SID_Allreduce(&n_halos_allocate, &n_halos_allocate_check, 1, SID_SIZE_T, SID_SUM, SID_COMM_WORLD);
