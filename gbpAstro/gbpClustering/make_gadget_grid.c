                ...
                for(i_type = 0; i_type < N_GADGET_TYPE; i_type++) {
                    for(i_particle = 0; i_particle < header.n_file[i_type]; i_particle += i_step) {
                        ...
                        for(i_buffer = 0; i_buffer < i_step; i_buffer++) {
                            ...
                            pos_test += (double)(1e3 * h_Hubble * ((double)vel_buffer[index]) /
                                                 (a_of_z(redshift) * M_PER_MPC * H_convert(H_z(redshift, cosmo))));
                            if(pos_test < 0)
                                pos_test += header.box_size;
                            if(pos_test >= header.box_size)
                                pos_test -= header.box_size;
...
int main(int argc, char *argv[]) {
    ...
    if(!strcmp(argv[5], "ngp") || !strcmp(argv[5], "NGP"))
        ...
        SID_exit_error("Invalid distribution scheme {%s} specified.", SID_ERROR_SYNTAX, argv[5]);
            ...
            for(i_grid = 0; i_grid < n_grid; i_grid++) {
                ...
                for(i_species = 0; i_species < plist.n_species; i_species++) {
                    if(flag_used[i_species]) {
                        ...
                        write_grid(
                            field[i_species], filename_out_species, i_write, n_grids_total, distribution_scheme, grid_identifier, header.box_size);
