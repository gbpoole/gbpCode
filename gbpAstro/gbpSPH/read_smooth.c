...
void read_smooth(plist_info *plist, char *filename_root_in, int snapshot_number, int mode) {
    char       filename[256];
            ...
            if(!flag_file_not_found) {
                ...
                if(flag_LONGIDs) {
                    for(i_particle = 0, j_particle = 0, n_mark = 0; i_particle < n_particles_file && j_particle < n_particles_local; i_particle++) {
                        while(j_particle < n_particles_local - 1 && ids[ids_index[j_particle]] < id_buf_L[id_buf_index[i_particle]])
                            j_particle++;
                    ...
                    for(i_particle = 0, j_particle = 0, n_mark = 0; i_particle < n_particles_file && j_particle < n_particles_local; i_particle++) {
                        while(j_particle < n_particles_local - 1 && ids[ids_index[j_particle]] < id_buf_i[id_buf_index[i_particle]])
                            j_particle++;
