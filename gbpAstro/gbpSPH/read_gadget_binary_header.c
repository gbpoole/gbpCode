...
void read_gadget_binary_header(char *filename_root_in, int snapshot_number, plist_info *plist) {
    ...
    if(flag_filefound) {
        ...
        SID_fread_verify(&record_length, 4, 1, fp);
        s_load = 0;

        //  ...particle numbers for each species...
        n_particles = 0;
        n_return    = SID_fread_verify(n_of_type_tmp, sizeof(unsigned int), N_GADGET_TYPE, fp);
        ...
        //  ...particle mass array...
        n_return = SID_fread_verify(mass_array, sizeof(double), N_GADGET_TYPE, fp);
        s_load += n_return * sizeof(double);

        //  ...expansion factor (or time)...
        n_return = SID_fread_verify(&d_value, sizeof(double), 1, fp);
        s_load += n_return * sizeof(double);
        ...
        //  ...some flags...
        n_return = SID_fread_verify(&i_value, sizeof(int), 1, fp);
        ...
        n_return = SID_fread_verify(n_all_tmp, sizeof(int), N_GADGET_TYPE, fp);
        ...
        n_return = SID_fread_verify(&box_size, sizeof(double), 1, fp);
        s_load += n_return * sizeof(double);
        ...
        //  ...Hubble parameter...
        n_return = SID_fread_verify(&h_Hubble, sizeof(double), 1, fp);
        s_load += n_return * sizeof(double);
        ...
        n_return = SID_fread_verify(&flag_metals, sizeof(int), 1, fp);
        s_load += n_return * sizeof(int);
        n_return = SID_fread_verify(&flag_ages, sizeof(int), 1, fp);
        s_load += n_return * sizeof(int);
        n_return = SID_fread_verify(&n_all_tmp, sizeof(int), N_GADGET_TYPE, fp);
        s_load += n_return * sizeof(int);
        n_return = SID_fread_verify(&flag_entropyICs, sizeof(int), 1, fp);
        s_load += n_return * sizeof(int);
        for(i = 0; i < N_GADGET_TYPE; i++)
