            ...
            if(n_particles > 0) {
                ...
                for(i = 0; i < plist->n_species; i++) {
                    if(flag_used[i]) {
                        ...
                        if(ADaPS_exist(plist->data, var_name)) {
                            calc_mean(ADaPS_fetch(plist->data, var_name), &mean, n_of_type[i], SID_DOUBLE, CALC_MODE_RETURN_DOUBLE);
                            mean /= M_SOL;
                            calc_median(ADaPS_fetch(plist->data, var_name), &median, n_of_type[i], SID_DOUBLE, CALC_MODE_RETURN_DOUBLE);
                            median /= M_SOL;
                            calc_min(ADaPS_fetch(plist->data, var_name), &min, n_of_type[i], SID_DOUBLE, CALC_MODE_RETURN_DOUBLE);
                            min /= M_SOL;
                            calc_max(ADaPS_fetch(plist->data, var_name), &max, n_of_type[i], SID_DOUBLE, CALC_MODE_RETURN_DOUBLE);
                            max /= M_SOL;
                            calc_stddev(ADaPS_fetch(plist->data, var_name), &std_dev, n_of_type[i], SID_DOUBLE, CALC_MODE_RETURN_DOUBLE);
                            std_dev /= M_SOL;
                ...
                if(n_of_type[GADGET_TYPE_GAS] > 0) {
                    ...
                    if(ADaPS_exist(plist->data, var_name)) {
                        ...
                        std_dev /= rho_crit;
                        fprintf(
                            stderr, "%11s  %11.3le  %11.3le  %11.3le  %11.3le  %11.3le [rho_crit(z)]\n", var_name, min, max, mean, median, std_dev);
                    }
