...
void read_mark_file(plist_info *plist, const char *mark_name, const char *filename_in, int mode) {
    ...
    for(i_species = 0, n_mark_local = 0, n_mark_total_check = 0; i_species < header.n_type; i_species++) {
        if(header.n_mark_species[i_species] > 0) {
            ADaPS_store(
                &(plist->data), (void *)(&(header.n_mark_species[i_species])), "n_%s_%s", ADaPS_SCALAR_SIZE_T, mark_name, plist->species[i_species]);
            ...
            ADaPS_store(&(plist->data),
                        (void *)(&(n_mark_type_local[i_species])),
                        "n_local_%s_%s",
                        ADaPS_SCALAR_SIZE_T,
                        mark_name,
        ...
        case MARK_LIST_ONLY:
            for(i_species = 0; i_species < header.n_type; i_species++) {
                if(header.n_mark_species[i_species] > 0) {
                    ...
                    // Perform read
                    SID_fread_chunked(mark_list_local, n_mark_type_local[i_species], i_start_local[i_species], &fp_mark_file);
                ...
                if(header.n_mark_species[i_species] > 0) {
                    ...
                    for(i_particle = 0; i_particle < header.n_mark_species[i_species];) {
                        ...
                        for(j_particle = 0, k_particle = find_index(ids_local, mark_list_buffer[0], n_particles_local, ids_local_index);
                            ...
                            while(ids_local[ids_local_index[k_particle]] < mark_list_local[j_particle] && k_particle < n_buffer - 1)
                                k_particle++;
