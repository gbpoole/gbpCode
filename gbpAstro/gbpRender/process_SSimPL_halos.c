...
void process_SSimPL_halos(render_info *render,
    ...
    if(render->trees == NULL)
        init_trees_read(
            render->filename_SSimPL_root, render->filename_halos_version, render->filename_trees_version, TREE_READ_HEADER_ONLY, &(render->trees));
    ...
    for(i_group = 0, i_subgroup = 0; i_group < n_groups; i_group++) {
        ...
        for(j_subgroup = 0; j_subgroup < group_i.n_subgroups; i_subgroup++, j_subgroup++) {
            ...
            if(select_function(i_group, j_subgroup, i_subgroup, flag_long_ids, &group_i, &subgroup_i, params)) {
                // Read IDs
                if(i_pass) {
                ...
                action_function(i_group, j_subgroup, i_subgroup, flag_long_ids, &group_i, &subgroup_i, params);
            }
        }
