...
void init_scene(scene_info **scene, int n_frames) {
    ...
    if(n_frames <= 0)
        SID_trap_error("Invalid number of frames (%d) requested for scene.", SID_ERROR_LOGIC, n_frames);
    ...
    (*scene)->sealed          = GBP_FALSE;
    (*scene)->next            = NULL;
    (*scene)->flag_time_set   = GBP_FALSE;
    (*scene)->flag_p_o_set    = GBP_FALSE;
    (*scene)->flag_radius_set = GBP_FALSE;
