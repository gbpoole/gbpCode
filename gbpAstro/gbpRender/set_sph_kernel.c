...
void set_sph_kernel(double **kernel_radius, double **kernel_table_3d, double **kernel_table_2d, double *kernel_table_2d_average, int mode) {
    ...
    if(!flag_skip) {
        ...
        if(fp_in != NULL) {
            int n_k_in;
            int mode_in;
            SID_fread_verify(&n_k_in, sizeof(int), 1, fp_in);
            SID_fread_verify(&mode_in, sizeof(int), 1, fp_in);
            if(n_k_in == N_KERNEL_TABLE && mode_in == mode) {
                ...
                SID_fread_verify((*kernel_radius), sizeof(double), N_KERNEL_TABLE + 1, fp_in);
                SID_fread_verify((*kernel_table_3d), sizeof(double), N_KERNEL_TABLE + 1, fp_in);
                if(SID_CHECK_BITFIELD_SWITCH(mode, SPH_KERNEL_2D)) {
                    (*kernel_table_2d) = (double *)SID_malloc(sizeof(double) * (N_KERNEL_TABLE + 1));
                    SID_fread_verify((*kernel_table_2d), sizeof(double), N_KERNEL_TABLE + 1, fp_in);
                    SID_fread_verify(kernel_table_2d_average, sizeof(double), 1, fp_in);
                }
            ...
            if(SID_CHECK_BITFIELD_SWITCH(mode, SPH_KERNEL_GADGET)) {
                ...
                for(i_table = 0; i_table <= N_KERNEL_TABLE; i_table++) {
                    if((*kernel_radius)[i_table] <= 0.5)
                        (*kernel_table_3d)[i_table] =
                            (8. / PI) * (1. - 6. * (*kernel_radius)[i_table] * (*kernel_radius)[i_table] * (1. - (*kernel_radius)[i_table]));
                    else if((*kernel_radius)[i_table] <= 1.)
                        (*kernel_table_3d)[i_table] =
                            (16. / PI) * (1. - (*kernel_radius)[i_table]) * (1. - (*kernel_radius)[i_table]) * (1. - (*kernel_radius)[i_table]);
                    else
                        (*kernel_table_3d)[i_table] = 0.;
                }
                ...
                for(i_table = 0; i_table <= N_KERNEL_TABLE; i_table++) {
                    for(j_table = i_table, n_temp = 0; j_table <= N_KERNEL_TABLE; j_table++, n_temp++) {
                        if(n_temp == 0)
                            ...
                            kernel_radius_temp[n_temp] =
                                sqrt((*kernel_radius)[j_table] * (*kernel_radius)[j_table] - (*kernel_radius)[i_table] * (*kernel_radius)[i_table]);
                    }
                    if(n_temp <= 1)
                    ...
                    if(i_table == 1)
                        (*kernel_table_2d_average) += PI * (*kernel_radius)[i_table] * (*kernel_radius)[i_table] * 0.5 *
                                                      ((*kernel_table_2d)[i_table - 1] + (*kernel_table_2d)[i_table]);
                    else if(i_table > 1)
                        (*kernel_table_2d_average) += TWO_PI * (*kernel_radius)[i_table] *
                                                      ((*kernel_radius)[i_table] - (*kernel_radius)[i_table - 1]) * 0.5 *
                                                      ((*kernel_table_2d)[i_table - 1] + (*kernel_table_2d)[i_table]);
                }
