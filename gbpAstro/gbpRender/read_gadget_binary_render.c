    ...
    if(flag_filefound) {
        ...
        if(SID_CHECK_BITFIELD_SWITCH(mode, READ_GADGET_RENDER_SCATTER)) {
            ...
            for(i_file = 0, k_particle = 0; i_file < n_files; i_file++) {
                ...
                int                flag_file_missing = GBP_FALSE;
        ...
        // WARNING: This assumes that all IDs are represented between 0 and n_particles_all-1
            ...
            for(i_rank = 0; i_rank < SID.n_proc; i_rank++) {
                ...
                if(n_particles_left > 0) {
                    ...
                    n_particles_step = GBP_MIN(n_particles_step, n_particles_left);
                    if(i_rank == 0)
                        id_local_min_i = 0;
                    else
        ...
        int flag_read_velocities = GBP_TRUE;
            ...
            if(!flag_file_empty && !flag_file_missing) {
                ...
                // Assign each rank a contiguous range of IDs
                    ...
                    if(SID.My_rank == read_rank) {
                        ...
                        // Read IDs
                        SID_fread_verify(&record_length_open, 4, 1, fp);
                        SID_fread_verify(buffer, record_length_open, 1, fp);
                        SID_fread_verify(&record_length_close, 4, 1, fp);
                        if(record_length_open != record_length_close)
                ...
                if(flag_read_positions) {
                    if(SID.My_rank == read_rank) {
                        SID_fread_verify(&record_length_open, 4, 1, fp);
                        SID_fread_verify(buffer2, (size_t) record_length_open, 1, fp);
                        SID_fread_verify(&record_length_close, 4, 1, fp);
                        if(record_length_open != record_length_close)
                ...
                if(flag_read_velocities) {
                    if(SID.My_rank == read_rank) {
                        SID_fread_verify(&record_length_open, 4, 1, fp);
                        SID_fread_verify(buffer2, record_length_open, 1, fp);
                        SID_fread_verify(&record_length_close, 4, 1, fp);
                        if(record_length_open != record_length_close)
