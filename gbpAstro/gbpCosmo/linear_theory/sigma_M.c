...
void init_sigma_M(cosmo_info **cosmo, int mode, int component) {
    ...
    if(!ADaPS_exist((*cosmo), d2ln_sigma_name)) {
        ...
        init_interpolate(lM_k, ln_sigma, (size_t)n_k, gsl_interp_cspline, &interp_ln_sigma);
        ADaPS_store_interp(cosmo, (void *)(interp_ln_sigma), d2ln_sigma_name);
        // ln(1/sigma)(lnM) interpolation
        init_interpolate(lM_k, ln_Inv_sigma, (size_t)n_k, gsl_interp_cspline, &interp_ln_Inv_sigma);
        ADaPS_store_interp(cosmo, (void *)(interp_ln_Inv_sigma), d2ln_Inv_sigma_name);
        // sigma(lnM) interpolation
        init_interpolate(lM_k, sigma_lnM, (size_t)n_k, gsl_interp_cspline, &interp_sigma_lnM);
        ADaPS_store_interp(cosmo, (void *)(interp_sigma_lnM), sigma_lnM_name);
        SID_log("Done.", SID_LOG_CLOSE);
