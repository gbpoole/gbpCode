...
void init_power_spectrum_variance(cosmo_info **cosmo, int mode, int component) {
    ...
    for(int i = 0; i < n_k; i++) {
        ...
        if(limit_lo < limit_hi) {
            while(rel_diff >= 0.25 * rel_accuracy) {
                ...
                double abs_error;
                gsl_integration_qag(
                    &integrand, limit_lo, limit_hi, abs_accuracy, rel_accuracy, n_int, GSL_INTEG_GAUSS15, wspace, &dintegral, &abs_error);
    ...
    ADaPS_store(cosmo, (void *)(sigma2), "sigma2_k_%s_%s", ADaPS_DEFAULT, mode_name, component_name);
    ADaPS_store_interp(cosmo, (void *)(interp), "sigma2_k_%s_%s_interp", mode_name, component_name);
