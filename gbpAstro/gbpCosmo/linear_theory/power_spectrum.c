...
void init_power_spectrum_TF(cosmo_info **cosmo) {
    ...
    if(!ADaPS_exist(*cosmo, "filename_transfer_function"))
        SID_exit_error("Transfer function filename has not been specified prior to calling init_power_spectrum_TF().",
                       SID_ERROR_LOGIC);
    ...
    */
    ...
    ADaPS_store(cosmo, (void *)(&n_k), "n_k", ADaPS_SCALAR_INT);
    ADaPS_store(cosmo, (void *)(lk_P), "lk_P", ADaPS_DEFAULT);
    ADaPS_store(cosmo, (void *)(lP_k), "lP_k_TF_all", ADaPS_DEFAULT);
    ADaPS_store(cosmo, (void *)(lP_k_gas), "lP_k_TF_gas", ADaPS_DEFAULT);
    ADaPS_store(cosmo, (void *)(lP_k_dark), "lP_k_TF_dark", ADaPS_DEFAULT);
    ...
    ADaPS_store_interp(cosmo, (void *)(interp), "lP_k_TF_all_interp");
    ADaPS_store_interp(cosmo, (void *)(interp_gas), "lP_k_TF_gas_interp");
    ADaPS_store_interp(cosmo, (void *)(interp_dark), "lP_k_TF_dark_interp");
...
double power_spectrum_normalization(cosmo_info *cosmo, int mode, int component) {
    ...
    gsl_integration_qag(&integrand, limit_lo, limit_hi, 0, rel_accuracy, n_int, GSL_INTEG_GAUSS61, wspace, &sigma2_unnorm, &abs_error);
...
double power_spectrum(double k_interp, double redshift, cosmo_info **cosmo, int mode, int component) {
    double rval;
    switch(mode) {
        ...
        default:
            SID_exit_error("Unsupported mode (%d) passed to power_spectrum().", SID_ERROR_LOGIC, mode);
            break;
    }
    return (rval);
}
