...
void heap_sort(void *data_in, size_t n_data, size_t **index, SID_Datatype data_type, int flag_compute_index, int flag_in_place) {
    size_t  i, ir, j, l;
    void *  rra;
    size_t  rrb;
    void *  data;
    ...
    if(flag_compute_index == SORT_COMPUTE_INDEX || flag_compute_index == SORT_COMPUTE_RANK) {
        ...
        fprintf(stderr,
                "ERROR (heap_sort): flag_compute_index {%d} must be SORT_COMPUTE_INDEX||SORT_COMPUTE_RANK||SORT_INPLACE_ONLY\n",
                flag_compute_index);
    }
    ...
    for(;;) {
        ...
        while(j <= ir) {
            if(data_type == SID_INT) {
                if(j < ir && ((int *)data)[j - 1] < ((int *)data)[j])
                    j++;
                ...
                if(j < ir && ((size_t *)data)[j - 1] < ((size_t *)data)[j])
                    j++;
                ...
                if(j < ir && ((float *)data)[j - 1] < ((float *)data)[j])
                    j++;
                ...
                if(j < ir && ((double *)data)[j - 1] < ((double *)data)[j])
                    j++;
    ...
    if(flag_compute_index == SORT_COMPUTE_RANK) {
        heap_sort(idx_sort, n_data, index, SID_SIZE_T, SORT_COMPUTE_INDEX, SORT_COMPUTE_INPLACE);
