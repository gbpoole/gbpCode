...
void sort(void *sval, size_t nval, size_t **index, SID_Datatype data_type, int flag_local, int flag_compute_index, int flag_in_place) {
    int     i_rank;
    int     j_rank;
    int     rank_send_to;
    ...
    //   ... use a heap_sort for most situations (it uses less RAM!)
        ...
        /*
            heap_sort(sval,
                  ...
                  flag_in_place);
        */
        merge_sort(sval, nval, index, data_type, flag_compute_index, flag_in_place);
    ...
    //   ... perform a global search returning ranks
        ...
        merge_sort(sval, nval, index, data_type, SORT_COMPUTE_INDEX, SORT_COMPUTE_NOT_INPLACE);
        ...
        merge_sort(sval, nval, &rank, data_type, SORT_COMPUTE_RANK, SORT_COMPUTE_NOT_INPLACE);
        ...
        if(flag_local == SORT_GLOBAL && SID.n_proc > 1) {
            ...
            for(i_rank = 1; i_rank < SID.n_proc; i_rank++) {
                ...
                exchange_ring_buffer(sval, data_type_size, nval, sval_tmp, &nval_tmp, i_rank);
                exchange_ring_buffer((*index), sizeof(size_t), nval, index_tmp, &nval_tmp, i_rank);
                ...
                if(data_type == SID_DOUBLE) {
                    ...
                    SID_exit_error("Unknown variable type in sort().", SID_ERROR_LOGIC);
        ...
        if(flag_local == SORT_GLOBAL && flag_compute_index != SORT_COMPUTE_RANK) {
            size_t *sort_ranks;
