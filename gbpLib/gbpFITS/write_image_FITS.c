...
int write_image_FITS(void *image, SID_Datatype dtype, int n_D, int *D_in, const char *filename, const char *ext_name) {
    ...
    if(status) {
        ffgmsg(error_msg);
        SID_exit_error("FITS create file: filename={%s} status=%d message={%s}", SID_ERROR_IO_OPEN, filename, status,
                       error_msg);
    }
    ...
    if(dtype == SID_FLOAT) {
        ...
        if(status) {
            ffgmsg(error_msg);
            SID_exit_error("FITS create image: filename={%s} status=%d message={%s}", SID_ERROR_IO_OPEN, filename,
                           status, error_msg);
        }
        fits_write_img(fp, TFLOAT, fpixel, n_pixels, image, &status);
        if(status) {
            ffgmsg(error_msg);
            SID_exit_error("FITS write image: filename={%s} status=%d message={%s}", SID_ERROR_IO_OPEN, filename,
                           status, error_msg);
        }
        bit_pix = FLOAT_IMG;
    }
    ...
    // Throw error if the given dtype is not supported
        ...
        SID_exit_error("Unsupported datatype in write_image_FITS", SID_ERROR_LOGIC);
