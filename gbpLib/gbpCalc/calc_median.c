...
void calc_median(void *data, void *result, size_t n_data, SID_Datatype type, int mode) {
    ...
    if(n_data < 1) {
        ...
        merge_sort(data, n_data, &index, type, SORT_COMPUTE_INDEX, FALSE);
        i_mid = n_data / 2;
        if(n_data % 2) {
            if(type == SID_DOUBLE)
            ...
            if(type == SID_DOUBLE)
                ...
                median = ONE_HALF * (double)(((float *)data)[index[i_mid - 1]] + ((float *)data)[index[i_mid]]);
            else if(type == SID_INT)
                median = ONE_HALF * (double)(((int *)data)[index[i_mid - 1]] + ((int *)data)[index[i_mid]]);
            else if(type == SID_SIZE_T)
