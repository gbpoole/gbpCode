...
macro(define_project_env_variable variableName description default_value )
    ...
    # Define an appropriate compile option from the variable (convert to 1-or-0 if clearly a boolean)
    if(_VAR_SET)
        set(boolean_list "ON" "OFF" "TRUE" "FALSE" "1" "0" ON OFF TRUE FALSE 1 0)
        list(FIND boolean_list "${${variableName}}" _index)
        if(_index GREATER -1)
            if(${${variableName}})
                option(${variableName} ${description} ON)
                add_definitions(-D${variableName}=1)
            else()
                option(${variableName} ${description} OFF)
